...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| hostList | Array&lt;string> | Yes | 绕过智能防跟踪功能的域名列表。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| avoidHeight | number | Yes | 设置Web网页可视视口底部避让高度。 单位：vp 合法取值范围：0~Web组件高度 非法值设置行为：小于0取值为0，大于Web组件高度取值为  Web组件高度。 |
...
按照历史栈，后退一个页面。一般结合[accessBackward]webview.WebviewController.accessBackward一起使用。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| hostName | string | Yes | 要清除DNS记录的主机域名。 |
...
清除所有通过[searchAllAsync]webview.WebviewController#searchAllAsync匹配到的高亮字符查找结果。
...
根据指定的缓存key列表清除对应的预获取资源缓存。入参中的缓存key必须是[prefetchResource]webview.WebviewController.prefetchResource指定预获取到的资 源缓存key。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| cacheKeyList | Array&lt;string> | Yes | 用于后续查询预获取资源缓存的key。仅支持字母和数字，未传入或传入空则取默认值url作为key。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| isExtentionType | boolean | No | 是否使用扩展增强接口。 默认值：false。[since 10] |
...
| --- | --- |
| 17100001 | Init error.  The WebviewController must be associated with a Web component. |
| 401 | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.  2. Incorrect parameter types. 3.Parameter verification failed. [since 10] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| schemes | Array&lt;WebCustomScheme> | Yes | 自定义协议配置，最多支持同时配置10个自定义协议。 |
...
| --- | --- |
| 401 | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.  2. Incorrect parameter types. |
| 17100020 | Failed to register custom schemes. [since 12] |
...
| --- | --- | --- | --- |
| schemes | Array&lt;WebCustomScheme> | Yes | 自定义协议配置，最多支持同时配置10个自定义协议。 |
| lazyInitWebEngine | boolean | Yes | 为true时：接口内部跳过初始化WebEngine。  临时存储注册的方案，当它实际被传递给WebEngine时，这些方案将被传递给WebEngine  初始化。当false时：接口内部自动进行WebEngine初始化  - 表示接口内部是否跳过初始化WebEngine。 true表示接口内部跳过初始化WebEngine，并将注册的Schemes暂存，当它真正初始化  时，这些Schemes将传递给WebEngine。false表示接口内部自动进行WebEngine初始化。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| name | string | Yes | 要删除的已注册JavaScript对象的名称。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enable | boolean | Yes | 是否启用广告过滤功能。 true表示启用广告过滤功能，false表示取消广告过滤功能。 默认值：false。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| securityParams | SecurityParams | Yes | 参数表示将禁用支持的选项或项目。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| features | BackForwardCacheSupportedFeatures | Yes | 允许使用特定的页面进入前进后退缓存中。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enable | boolean | Yes | 是否启用智能防跟踪功能。 true表示启用智能防跟踪功能，false表示不启用智能防跟踪功能。 默认值：false。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enable | boolean | Yes | {@code true} 启用检查网站安全风险的功能， {@code false} 表示不启用检查网站安全风险的功能。 |
...
| Type | Description |
| --- | --- |
| ArkWebEngineVersion | 返回由[ArkWebEngineVersion](arkts-webview-arkwebengineversion-e.md#ArkWebEngineVersion)所定义的当  前使用的ArkWeb内核版本。 |
...
获取页面首屏加载预测信息（详细说明见[BlanklessInfo](arkts-webview-blanklessinfo-i.md#BlanklessInfo)），并开始本次加载过渡帧生成，应用根据此信息确定是否需要启用无白屏 加载。必须与[setBlanklessLoadingWithKey]webview.WebviewController.setBlanklessLoadingWithKey接口配套使用，并且必须在触发加载页面的 接口之前或在`onLoadIntercept`中调用。需在`WebViewController`与Web组件绑定后才能使用。 > **说明：** > > - 持久缓存容量：默认大小为30MB（约30页），可以通过接口 > [setBlanklessLoadingCacheCapacity]webview.WebviewController.setBlanklessLoadingCacheCapacity设置缓存容量，具体见该 > 接口说明。超过容量时根据LRU（Least Recently Used，淘汰不常用缓存的策略）机制更新缓存。自动清理超过7天的持久缓存数据，缓存清除后第三次加载页面开始有优化效果。 > > - 如果发现快照相似度（即[BlanklessInfo](arkts-webview-blanklessinfo-i.md#BlanklessInfo)中的similarity）极低，请确认key值是否传递正确。 > > - 调用本接口后，将启用页面加载快照检测及生成过渡帧计算，会产生一定的资源开销。 > > - 启用无白屏加载的页面会带来一定的资源开销，开销的大小与Web组件的分辨率相关。假设分辨率的宽度和高度分别为：w, h。页面在打开阶段会增加峰值内存，增加约12 * w * h B，页面打开后内存回收，不影响稳态内存。增 > 加固态应用缓存的大小，每个页面增加的缓存约w * h / 10 B，缓存位于应用缓存的位置。 > > - 请在module.json5中添加权限: ohos.permission.INTERNET和ohos.permission.GET_NETWORK_INFO，具体权限的添加方法请参考 > [在配置文件中声明权限](docroot://security/AccessToken/declare-permissions.md#在配置文件中声明权限)。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| key | string | Yes | 唯一标识本页面的key值。 合法取值范围：非空，长度不超过2048个字符。 设置非法值时不生效。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;Array&lt;cert.X509Cert>> | Yes | 通过AsyncCallback异步返回当前网站的X509格式证书。 |
...
获取当前被点击区域的元素类型。 > **说明：** > > 从API version11开始支持，从API version 18开始废弃。建议使用[getLastHitTest]webview.WebviewController#getLastHitTest替代。

**Since:** 9

**Deprecated since:** 18

**Substitute:** webview.WebviewController#getLastHitTest
...
获取当前被点击区域的元素信息。 > **说明：** > > 从API version11开始支持，从API version 18开始废弃。建议使用[getLastHitTest]webview.WebviewController#getLastHitTest替代。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;boolean> | Yes | 返回查找页面是否存在图像。 true表示页面存在图像；false表示页面不存在图像。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| resourceMaps | Array&lt;OfflineResourceMap> | Yes | 本地离线资源配置对象，单次调用最大支持注入30个资源，单个资源最大支持10Mb。 |
...
| 401 | Parameter error.  Possible causes: 1. Mandatory parameters are left unspecified.  2. Incorrect parameter types. 3. Parameter verification failed. |
| 17100001 | Init error.  The WebviewController must be associated with a Web component. |
| 17100002 | URL error. The webpage corresponding to the URL is invalid, or the URL  length exceeds 2048. [since 12 - 21] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| data | string | Yes | 按照"base64"或者"URL"编码后的一段字符串。 |
| mimeType | string | Yes | 媒体类型（MIME）。 |
| encoding | string | Yes | 编码类型，具体为"base64"或者"URL"编码。 |
| baseUrl | string | No |  |
...
| 401 | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.  2. Incorrect parameter types. 3.Parameter verification failed. |
| 17100001 | Init error.  The WebviewController must be associated with a Web component. |
| 17100002 | URL error. The webpage corresponding to the URL is invalid, or the URL  length exceeds 2048. [since 9 - 10] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| url | string \| Resource | Yes | 需要加载的URL。 |
...
注册回调函数，开启 [应用接管网页媒体播放功能](docroot://reference/apis-arkweb/arkts-basic-components-web-attributes.md#enablenativemediaplayer12) 后，当网页中有播放媒体时，触发注册的回调函数。 如果应用接管网页媒体播放功能未开启，则注册的回调函数不会被触发。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | CreateNativeMediaPlayerCallback | Yes | 接管网页媒体播放的回调函数。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| bottom | boolean | Yes | 是否跳转到页面最底部。 false时表示将页面内容向下滚动半个视框大小，true表示跳转到页面最底部。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| top | boolean | Yes | 是否跳转到页面最顶部。 false表示将页面内容向上滚动半个视框大小，true表示跳转到页面最顶部。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| name | string | Yes | 要发送的消息名称。 |
| ports | Array&lt;WebMessagePort> | Yes | 要发送的消息端口。 |
| uri | string | Yes | 接收该消息的URI。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| url | string | Yes | 需要加载的URL。 |
| postData | ArrayBuffer | Yes | 使用"POST"方法传递数据。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| url | string | Yes | 本地JavaScript文件对应的网络地址，即业务网页请求该文件的服务器版本时使用的网络地址。网络地址仅支持http或https协议，长度不超过2048。如果该网络地址对应的缓存  失效，则业务网页将通过网络请求对应的资源。 |
| script | string \| Uint8Array | Yes | 本地JavaScript的文本内容。内容不能为空。 |
| cacheOptions | CacheOptions | Yes | 用于控制字节码缓存更新。 |
...
| --- | --- |
| 17100001 | Init error.  The WebviewController must be associated with a Web component. |
| 17100002 | URL error. The webpage corresponding to the URL is invalid, or the URL  length exceeds 2048. [since 10 - 21] |
...
| url | string | Yes | 预加载的URL。 |
| additionalHeaders | Array&lt;WebHeader> | No |  |
| prefetchOptions | PrefetchOptions | No |  |
...
| --- | --- |
| 17100001 | Init error. The WebviewController must be associated with a Web component. |
| 17100002 | URL error. The webpage corresponding to the URL is invalid, or the URL  length exceeds 2048. [since 21 - 21] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| request | RequestInfo | Yes | 预获取请求的信息。 |
| additionalHeaders | Array&lt;WebHeader> | No |  |
| cacheKey | string | No |  |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| url | string | Yes | 预连接的URL。 |
| preconnectable | boolean | Yes | 是否进行预连接。如果preconnectable为true，则对URL进行DNS解析，socket建链预连接；如果preconnectable为false  ，则不做任何预连接操作。 |
| numSockets | number | Yes | 要预连接的socket数。socket数目连接需要大于0，最多允许6个连接。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| ignoreCache | boolean | Yes | Web组件刷新网页，选择是否忽略缓存刷新。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| jsObject | object | Yes | Application side JavaScript objects participating in registration. [since 9] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| clearRom | boolean | Yes | 设置为true时同时清除ROM和RAM中的缓存，设置为false时只清除RAM中的缓存。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| state | Uint8Array | Yes | 页面状态历史记录序列化数据。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| script | string | Yes | JavaScript脚本。 |
...
| --- | --- | --- | --- |
| script | string | Yes | JavaScript脚本。 |
| callback | AsyncCallback&lt;string> | Yes | 回调执行JavaScript脚本结果。JavaScript脚本若执行失败或无返回值时，返回null。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| script | string \| ArrayBuffer | Yes | JavaScript脚本。 [since 10 - 11] |
...
| --- | --- | --- | --- |
| script | string \| ArrayBuffer | Yes | JavaScript脚本。 [since 10 - 11] |
| callback | AsyncCallback&lt;JsMessageExt> | Yes | 回调执行JavaScript脚本结果。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| deltaX | number | Yes | 水平偏移量，其中水平向右为正方向。 单位：vp。 |
| deltaY | number | Yes | 垂直偏移量，其中垂直向下为正方向。 单位：vp。 |
| duration | number | No | 滚动动画时间。 单位：ms。  不传入为无动画，当传入数值为负数或传入0时，按照不传入处理。  传入null或undefined时会抛出异常错误码401。 [since 14] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| x | number | Yes | 绝对位置的水平坐标，当传入数值为负数时，按照传入0处理。 单位：vp。 |
| y | number | Yes | 绝对位置的垂直坐标，当传入数值为负数时，按照传入0处理。 单位：vp。 |
| duration | number | No |  |
...
异步查找网页中所有匹配关键字'searchString'的内容并高亮，结果通过 [onSearchResultReceive]@ohos.web.WebAttribute#onsearchresultreceive异步返回。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| searchString | string | Yes | 查找的关键字。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| forward | boolean | Yes | 从前向后或者逆向查找方式。 true表示从前向后查找，false表示从后向前查找。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| engineVersion | ArkWebEngineVersion | Yes | ArkWeb内核版本。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| mute | boolean | Yes | 表示是否将网页设置为静音状态。 true表示将网页设置为静音状态，false表示将网页取消静音状态。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| options | BackForwardCacheOptions | Yes | 用来控制Web组件前进后退缓存相关选项。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| capacity | number | Yes | 设置持久化缓存设置，单位MB，最大设置不超过100MB。 合法取值范围：[0, 100]，当设置为0时，无缓存空间，则功能全局不开启。 非法值设置行为：小于  0时生效值为0，大于100时生效值为100。 |
...
设置无白屏加载是否启用，本接口必须与[getBlanklessInfoWithKey]webview.WebviewController.getBlanklessInfoWithKey接口配套使用。 > **说明：** > > - 需在触发页面加载的接口之后调用，其他约束同[getBlanklessInfoWithKey]webview.WebviewController.getBlanklessInfoWithKey。 > > - 页面加载必须在调用本接口的组件中进行。 > > - 当相似度较低时，系统将判定为跳变过大，启用插帧会失败。 > > - 请在module.json5中添加权限: ohos.permission.INTERNET和ohos.permission.GET_NETWORK_INFO，具体权限的添加方法请参考 > [在配置文件中声明权限](docroot://security/AccessToken/declare-permissions.md#在配置文件中声明权限)。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| key | string | Yes | 唯一标识本页面的key值。必须与getBlanklessInfoWithKey接口的key值相同。 合法取值范围：非空，长度不超过2048个字符。 非法值设置行为：返  回错误码WebBlanklessErrorCode，方案不生效。 |
| is_start | boolean | Yes | 是否启用开始插帧。true：启用，false：不启用。 传入undefined或null时为false。 |

**Return value:**

| Type | Description |
| --- | --- |
| WebBlanklessErrorCode | 返回接口调用是否成功，具体见  [WebBlanklessErrorCode](arkts-webview-webblanklesserrorcode-e.md#WebBlanklessErrorCode)定义。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| key | string | Yes | Key value that uniquely identifies the current page.  Value range: (0, 2048]  The key value must be the same as that of getBlanklessInfoWithKey. |
| param | BlanklessLoadingParam | Yes | The blankless loading parameter.  None |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| timeout | number | Yes | socket连接超时时间，以秒为单位，必须为大于0的整数。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| userAgent | string | Yes | 用户自定义代理信息。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| hostName | string | Yes | 要添加DNS记录的主机域名。 |
| address | string | Yes | 主机域名解析地址（支持IPv4，IPv6）。 |
| aliveTime | number | Yes | 缓存有效时间（秒）。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| secureDnsMode | SecureDnsMode | Yes | 使用HTTPDNS的模式。 |
| secureDnsConfig | string | Yes | HTTPDNS server的配置，必须是https协议并且只允许配置一个server。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enable | boolean | Yes | 设置JavaScript中的`window.navigator.onLine`属性。 true表示设置JavaScript中的`  window.navigator.onLine`属性为true，false表示设置JavaScript中的`window.navigator.onLine`属性为false。 默认值：true。 |
...
设置一个路径列表，当file协议访问该路径列表中的资源时，允许跨域访问本地文件，也允许跨域访问其他在线资源。此外，当设置了路径列表时，file协议仅允许访问路径列表中的资源（ [fileAccess](docroot://reference/apis-arkweb/arkts-basic-components-web-attributes.md#fileaccess)的行为将会被此接口行为覆盖）。 setPathAllowingUniversalAccess放开目录的跨域访问限制是一个高风险操作。基于最小权限原则，当前el1，el2放开的路径是固定的，路径列表中的路径应符合以下任一路径格式： 1.应用文件目录的子目录（应用文件目录通过Ability Kit中的 [Context.filesDir](docroot://reference/apis-ability-kit/js-apis-inner-application-context.md#属性)获取），例如： * /data/storage/el2/base/files/example * /data/storage/el2/base/haps/entry/files/example 2.应用资源目录及其子目录（应用资源目录通过Ability Kit中的 [Context.resourceDir](docroot://reference/apis-ability-kit/js-apis-inner-application-context.md#属性)获取），例如： * /data/storage/el1/bundle/entry/resources/resfile * /data/storage/el1/bundle/entry/resources/resfile/example 3.从API version 21开始，还包括了应用缓存目录及其子目录（应用缓存目录通过Ability Kit中的 [Context.cacheDir](docroot://reference/apis-ability-kit/js-apis-inner-application-context.md#属性)获取），例如： * /data/storage/el2/base/cache * /data/storage/el2/base/haps/entry/cache/example * 设置的目录路径中，不允许包含cache/web，否则会抛出异常码401。如果设置目录路径是cache，cache/web也不允许访问。 4.从API version 21开始，还包括了应用临时目录及其子目录（应用临时目录通过Ability Kit中的 [Context.tempDir](docroot://reference/apis-ability-kit/js-apis-inner-application-context.md#属性)获取），例如： * /data/storage/el2/base/temp * /data/storage/el2/base/haps/entry/temp/example 当路径列表中有其中一个路径不满足以上条件之一，则会抛出异常码401，并且设置路径列表失败。当设置的路径列表为空，则file协议可访问范围以 [fileAccess](docroot://reference/apis-arkweb/arkts-basic-components-web-attributes.md#fileaccess)的行为为准。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| pathList | Array&lt;string> | Yes | 路径列表 |
...
| Error Code ID | Error Message |
| --- | --- |
| 401 | Parameter error. Possible causes: 1. Mandatory parameters are left unspecified.  2. Incorrect parameter types.  @static |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enable | boolean | Yes | 表示是否将网页设置为允许滚动。 true表示设置为允许滚动，false表示禁止滚动。 默认值：true。 |
| type | ScrollType | No | 网页可触发的滚动类型，支持缺省配置。  - enable为false时，表示禁止ScrollType类型的滚动，当ScrollType缺省时表示禁止所有类型网页滚动。  - enable为true时，ScrollType缺省与否，都表示允许所有类型的网页滚动。  传入null或undefined时会抛出异常错误码401。 |
...
在Web页面场景，设置全局滚动条模式。不显式调用时，默认为 [ScrollbarMode.OVERLAY_LAYOUT_SCROLLBAR](arkts-webview-scrollbarmode-e.md#ScrollbarMode)（非常驻滚动条）。 > **说明：** > > - 根据滚动条模式，改变当前应用所有web滚动条模式为常驻滚动条或非常驻滚动条。 > > - 若[forceDisplayScrollBar]@ohos.web.WebAttribute#forcedisplayscrollbar > 接口与当前接口同时设置，forceDisplayScrollBar接口设置不生效。 > > - 该接口需要在WebViewController绑定Web组件之前调用。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| scrollbarMode | ScrollbarMode | Yes | 滚动条模式。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| mode | SiteIsolationMode | Yes | The site isolation mode of the application,  default value depends on different devices type. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| timeout | number | Yes | ArkWeb中已经使用过的空闲socket的超时时间。 取值范围：[30,300]，单位：s。 小于30时生效值为30，大于300时生效值为300。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| webDebuggingAccess | boolean | Yes | 设置是否启用网页调试功能。 true表示启用网页调试功能。false表示不启用网页调试功能。 默认值：false。 |
...
设置是否启用无线网页调试功能，默认不开启。 当没有指定端口port时，该接口等同于 [setWebDebuggingAccess]webview.WebviewController.static setWebDebuggingAccess(webDebuggingAccess: boolean) 接口，ArkWeb会启动一个本地domain socket监听。 当指定了端口port时，ArkWeb会启动一个tcp socket监听。这时可以无线调试网页。 由于小于1024的端口号作为熟知或系统端口，在操作系统上需要特权才能开启，因此port的取值必须大于1024，否则该接口会抛出异常。 安全提示：启用网页调试功能可以让用户检查修改Web页面内部状态，存在安全隐患，不建议在应用正式发布版本中启用。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| webDebuggingAccess | boolean | Yes | 设置是否启用网页调试功能。 true表示开启网页调试功能，false表示关闭网页调试功能。 |
| port | number | Yes | 表示 devtools 服务器的端口。指定端口后，将创建一个 TCP 服务器套接字，而不是 Unix 域套接字。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| mode | WebDestroyMode | Yes | web组件销毁模式，默认NORMAL_MODE  @static |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| vx | number | Yes | 轻扫滚动的水平速度分量，其中水平向右为速度正方向。 单位：vp/s。 |
| vy | number | Yes | 轻扫滚动的垂直速度分量，其中垂直向下为速度正方向。 单位：vp/s。 |
...
| --- | --- |
| 17100001 | Init error.  The WebviewController must be associated with a Web component. |
| 17100002 | URL error. The webpage corresponding to the URL is invalid, or the URL  length exceeds 2048. [since 11 - 21] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| baseName | string | Yes | 生成的离线网页存储位置，该值不能为空。 |
| autoName | boolean | Yes | 决定是否自动生成文件名。 false表示按baseName的文件名存储，true表示根据当前URL自动生成文件名，并按baseName的文件目录存储。 |
...
| baseName | string | Yes | 生成的离线网页存储位置，该值不能为空。 |
| autoName | boolean | Yes | 决定是否自动生成文件名。 false表示按baseName的文件名存储，true表示根据当前URL自动生成文件名，并按baseName的文件目录存储。 |
| callback | AsyncCallback&lt;string> | Yes | 返回文件存储路径，保存网页失败会返回null。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| level | PressureLevel | Yes | 需要清理内存的内存等级。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| timeout | number | Yes | 异步等待时长。取值范围: [0, 65535]单位: ms |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| url | string | Yes | 需要预热ServiceWorker的URL。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| info | SnapshotInfo | Yes | 全量绘制结果入参。 |
| callback | AsyncCallback&lt;SnapshotResult> | Yes | 全量绘制回调结果。 |
...
调整当前网页的缩放比例，[zoomAccess]@ohos.web.WebAttribute#zoomAccess需为true.
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| factor | number | Yes | 基于当前网页所需调整的相对缩放比例，入参要求大于0，  当入参为1时为默认加载网页的缩放比例，入参小于1为缩小，入参大于1为放大。  取值范围：(0，100]。 |
