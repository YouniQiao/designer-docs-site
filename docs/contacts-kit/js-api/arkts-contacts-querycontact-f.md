...
| --- | --- | --- | --- |
| key | string | Yes | Unique query key of a contact, which is the unique identifier automatically generated bythe system when a contact is created. Each contact corresponds to one key, which can be obtained through[queryKey](arkts-contacts-querykey-f.md#querykey-2). |
| callback | AsyncCallback&lt;Contact&gt; | Yes | Indicates the callback for getting the result of the call. If the operation is successful, anarray of queried contacts is returned. If the operation fails, an error code is returned. |
