...
| 10200016 | The task to cancel is being executed. [since 9 - 17] |
| 10200055 | The asyncRunner task has been canceled. [since 18] |
...
取消任务池中的任务组。如果任务组中的任务未全部执行结束，返回**undefined**。 从API version 20开始，在执行cancel操作后，可以在catch分支里使用泛型BusinessError< [taskpool.TaskResult]taskpool.TaskResult>，来获取任务中抛出的异常信息或最终的执行结果。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| group | TaskGroup | Yes | 需要取消执行的任务组。 |
