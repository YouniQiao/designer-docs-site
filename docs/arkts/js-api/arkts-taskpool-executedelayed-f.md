...
| --- | --- | --- | --- |
| delayTime | number | Yes | 延时时间，单位为ms。delayTime值必须要大于等于0。  该值应为整数。  单位：毫秒。 |
| task | Task | Yes | 需要延时执行的任务。 |
...
| 10200006 | An exception occurred during serialization. [since 12] |
| 10200014 | The function is not marked as concurrent. [since 12] |
...
| --- | --- | --- | --- |
| delayTime | number | Yes | 延时时间，单位为ms。delayTime值必须要大于等于0。  该值应为整数。  单位：毫秒。 |
| task | GenericsTask&lt;A, R> | Yes | 需要延时执行的泛型任务。 |
| priority | Priority | No |  |
