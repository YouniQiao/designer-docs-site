...
| --- | --- | --- | --- |
| config | CmsEnvelopedDecryptionConfig | Yes | CMS解封装的配置。 |
...
| 19020003 | 参数检查失败。可能的原因：  1. 私钥无效或不支持；  2. 接收者证书无效或不支持。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| type | CmsCertType | Yes | 从cms中获取证书的类型。 |
...
| 19020001 | 内存错误。 |
| 19020002 | 运行时外部错误。可能的原因：  1. 内存拷贝失败；  2. 系统内部出现空指针；  3. 获取Native对象失败或参数转换失败。 |
| 19020003 | 参数检查失败。可能的原因：  1. type类型无效或不支持。 |
| 19030001 | 调用三方算法库API出错。 |
...
| cmsFormat | CmsFormat | Yes | 指定输入的CMS格式。 |
...
| 19020001 | 内存错误。 |
| 19020002 | 运行时外部错误。可能的原因：  1. 内存拷贝失败；  2. 系统内部出现空指针；  3. 获取Native对象失败或参数转换失败。 |
| 19020003 | 参数检查失败。可能的原因：  1. 数据长度为零或过大；  2. cmsFormat类型无效或不支持。 |
| 19030001 | 调用三方算法库API出错。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| config | CmsVerificationConfig | Yes | CMS验签配置内容。 |
...
| 19020001 | 内存错误。 |
| 19020002 | 运行时外部错误。可能的原因：  1. 内存拷贝失败；  2. 系统内部出现空指针；  3. 获取Native对象失败或参数转换失败。 |
| 19020003 | 参数检查失败。可能的原因：  1. config的trustCerts为空；  2. config的contentData长度为零或过大；  3. config的contentDataFormat无效或不支持。 |
| 19030001 | 调用三方算法库API出错。 |
| 19030003 | 证书尚未生效。 |
| 19030004 | 证书过期。 |
| 19030005 | 无法获取证书的颁发者。 |
