...
| Name | Description |
| --- | --- |
| [createConnection](arkts-distributedservice-createconnection-f.md#createconnection-1) | Creates a **Connection** object on the device that functions as the client. The device can then initiate connectionrequests to the device that functions as the server. |
...
| [Server](arkts-distributedservice-server-i.md) | Represents a **Server** object, which provides methods for starting, stopping, and closing the server, andregistering or unregistering event callbacks. |
