...
| --- | --- | --- | --- |
| plugin | DlpConnPlugin | Yes | Callback plugin object, which is used to register the callback capabilitywith the SA. The **DlpConnPlugin** API needs to be inherited and the **connectServer** method needs to beimplemented so that the processing result can be returned using a callback when the API is called on theSA. |
