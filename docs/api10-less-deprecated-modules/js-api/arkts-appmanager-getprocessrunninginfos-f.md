...
获取有关运行进程的信息。使用Promise异步回调。 > 从 API Version 9 开始废弃，建议使用 > [appManager.getRunningProcessInformation](../../apis-ability-kit/arkts-apis/arkts-appmanager-getrunningprocessinformation-f.md#getRunningProcessInformation-1) > 替代。
...
获取有关运行进程的信息。使用callback异步回调。 > 从 API Version 9 开始废弃，建议使用 > [appManager.getRunningProcessInformation](../../apis-ability-kit/arkts-apis/arkts-appmanager-getrunningprocessinformation-f.md#getRunningProcessInformation-1) > 替代。
...
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;Array&lt;ProcessRunningInfo>> | Yes | 回调函数，返回有关运行进程的信息。 |
