...
| Name | Description |
| --- | --- |
| [write](arkts-performanceanalysis-hiappevent-write-depr-f.md#write-1) | Writes event information to the event file of the current day. This API uses a promise to return the result. |
| [write](arkts-performanceanalysis-hiappevent-write-depr-f.md#write-2) | Writes event information to the event file of the current day. This API uses an asynchronous callback to return the result. |
| [configure](arkts-performanceanalysis-hiappevent-configure-depr-f.md#configure-1) | Configures the application event logging function, such as setting the event logging switch and maximum size of the directory that stores the event logging files. |
