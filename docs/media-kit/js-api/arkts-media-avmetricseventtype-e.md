...
## AV_METRICS_EVENT_CODEC_ABNORMAL

```TypeScript
AV_METRICS_EVENT_CODEC_ABNORMAL = 7
```

Audio/video decoder exception event, which is reported when an exception occurs in the decoder. The carried information includes the decoder type and exception type.
