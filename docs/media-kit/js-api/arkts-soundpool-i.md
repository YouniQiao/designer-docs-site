...
| callback | AsyncCallback&lt;int> | Yes | 异步音频资源加载返回的资源id，有效值大于0。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| fd | int | Yes | 资源句柄，通过  [resourceManager.getRawFd](docroot://reference/apis-localization-kit/js-apis-resource-manager.md)  获取。 |
| offset | long | Yes | 资源偏移量，需要基于预置资源的信息输入，非法值会造成音视频资源解析错误。 |
| length | long | Yes | 资源长度，需要基于预置资源的信息输入，非法值会造成音视频资源解析错误。 |
| callback | AsyncCallback&lt;int> | Yes | 获取回调的soundID，有效值大于0。 |
...
| callback | Callback&lt;ErrorInfo> | No |  |
...
| callback | Callback&lt;int> | Yes | 回调函数，返回对应资源加载完成的资源ID。 |
...
| callback | Callback&lt;void> | Yes | 异步'playFinished'的回调方法。 |
...
| callback | ErrorCallback | Yes | 错误事件回调方法：使用播放器的过程中发生错误，会提供错误码ID和错误信息。 |
...
| callback | Callback&lt;int> | Yes | 回调函数，返回播放完成的音频的streamId。 |
...
| callback | Callback&lt;ErrorInfo> | Yes | 回调函数，返回错误事件回调方法。在使用播放器的过程中发生错误时，提供错误信息[ErrorInfo](arkts-errorinfo-i.md#ErrorInfo)。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | Callback&lt;ErrorInfo> | Yes | Callback used to listen for soundpool errorOccurred events. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | Callback&lt;int> | Yes | ID of the sound that has been loaded. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | Callback&lt;int> | Yes | Callback used to return the result. Stream ID of the audio that  finishes playing. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| soundID | int | Yes | 资源ID，通过load方法获取。 |
| params | PlayParameters | Yes | play播放相关参数的设置。 |
| callback | AsyncCallback&lt;int> | Yes | 获取回调的音频流ID，有效值大于0。 |
...
| --- | --- | --- | --- |
| soundID | int | Yes | 资源ID，通过load方法获取。 |
| params | PlayParameters | No | play播放相关参数的设置。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当音频池release方法回调成功，err为undefined，否则为错误对象。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| streamID | int | Yes | 音频流ID，通过play方法获取。 |
| loop | int | Yes | 设置循环次数。 当loop≥0时，实际播放次数为loop+1。 当loop＜0时，表示一直循环。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当setLoop的回调成功，err为undefined，否则为错误对象。 |
...
| --- | --- | --- | --- |
| streamID | int | Yes | 音频流ID，通过play方法获取。 |
| priority | int | Yes | 优先级，0表示最低优先级。设置范围为大于等于0的整数。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当音频池setPriority方法回调成功，err为undefined，否则为错误对象。 |
...
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当音频池setRate方法回调成功，err为undefined，否则为错误对象。 |
...
| --- | --- | --- | --- |
| streamID | int | Yes | 音频流ID，通过play方法获取。 |
| leftVolume | double | Yes | 左声道音量，设置范围为[0.0, 1.0]。 |
| rightVolume | double | Yes | 右声道音量，设置范围为[0.0, 1.0]，当前右声道设置无效，以左声道为准。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当音频池setVolume方法回调成功，err为undefined，否则为错误对象。 |
...
| --- | --- | --- | --- |
| streamID | int | Yes | 音频流ID，通过play方法获取。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当音频池stop回调成功，err为undefined，否则为错误对象。 |
...
| --- | --- | --- | --- |
| soundID | int | Yes | 资源ID，通过load方法获取。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当音频池unload方法回调成功，err为undefined，否则为错误对象。 |
