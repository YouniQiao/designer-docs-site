...
**Inheritance:** SearchControllerextends: TextContentControllerBase.
...
| --- | --- | --- | --- |
| value | number | Yes | 从字符串开始到光标所在位置的长度。当value字符串长度时，按照字符串长度处理。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| selectionStart | number | Yes | 文本选择区域起始位置，文本框中文字的起始位置为0。 当selectionStart小于0时、按照0处理；当selectionStart大于文字最大长度时  、按照文字最大长度处理。 |
| selectionEnd | number | Yes | 文本选择区域结束位置。 当selectionEnd小于0时、按照0处理；当selectionEnd大于文字最大长度时、按照文字最大长度处理。 |
