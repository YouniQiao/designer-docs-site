...
| --- | --- | --- | --- |
| index | number | Yes | 指定页面在Swiper中的索引值。 说明： 设置的值小于0或大于最大页面索引时，取0。 |
| useAnimation | boolean | No | 设置翻至指定页面时是否有动效，true表示有动效，false表示没有动效。 默认值：false。 |
...
| --- | --- | --- | --- |
| index | number | Yes | 指定页面在Swiper中的索引值。 说明： 设置的值小于0或大于最大页面索引时，取0。 |
| animationMode | SwiperAnimationMode \| boolean | No |  |
...
设置模拟拖拽的拖拽距离。 > **说明：** > - 模拟拖拽的距离需要依赖布局体现，建议接口在布局前调用，拖拽效果可以在当前帧布局后体现。如果在未布局前调用了多次该接口，当前帧布局时只生效最后一次调用传入的拖拽距离。 > > - 在[loop](arkts-swiperattribute-c.md#loop)设置为true的循环场景下，如果设置的模拟拖拽的距离大于布局总长度，此时模拟拖拽距离会被调整为拖拽到刚好显示第一个子节点（向布局起点拖拽）或者最后一个子 > 节点（向布局终点方向拖拽）的距离。 > > - [onGestureSwipe](arkts-swiperattribute-c.md#onGestureSwipe)事件、 > [onContentWillScroll](arkts-swiperattribute-c.md#onContentWillScroll)事件在拖拽过程中不触发。 > [customContentTransition](arkts-swiperattribute-c.md#customContentTransition)会在布局前触发，由于真实的拖拽距离可能在布局时被调整，在传入拖拽距离过大时，触发事 > 件时的返回的节点显示信息可能与布局结果不一致。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| offset | number | Yes | 需要模拟拖拽的拖拽距离。 正数表示向布局起点拖拽；负数表示向布局终点方向拖拽。  单位为：vp。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | VoidCallback | No | 动画结束的回调。 [since 18] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| indices | Optional&lt;Array&lt;number>> | Yes | 需预加载的子节点的下标数组。 |
...
翻至下一页。翻页带动效切换过程，时长通过Swiper的[duration](arkts-swiperattribute-c.md#duration)属性设置。
...
翻至上一页。翻页带动效切换过程，时长通过Swiper的[duration](arkts-swiperattribute-c.md#duration)属性设置。
