...
**Inheritance:** CanvasRenderingContext2Dextends: CanvasRenderer.
...
| --- | --- | --- | --- |
| settings | RenderingContextSettings | No | 用来配置CanvasRenderingContext2D对象的参数，  见[RenderingContextSettings](#renderingcontextsettings)。  异常值undefined和null按[RenderingContextSettings](#renderingcontextsettings)的默认值处理。 |
...
从一个DrawingRenderingContext对象中获取一个CanvasRenderingContext2D对象， 该CanvasRenderingContext2D对象与入参的DrawingRenderingContext对象绑定了相同的Canvas组件。 > **说明：** > > - 从该接口获取的CanvasRenderingContext2D对象不允许作为参数创建 > [Canvas](docroot://reference/apis-arkui/arkui-ts/ts-components-canvas-canvas.md) > 组件，否则会导致应用崩溃。 > > - 当入参的DrawingRenderingContext对象未绑定Canvas组件时，将返回错误码。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| drawingContext | DrawingRenderingContext | Yes | 一个DrawingRenderingContext类型的对象。  异常值undefined或null按无效值处理。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| type | 'onAttach' | Yes | 取消订阅CanvasRenderingContext2D与Canvas组件发生绑定的回调。  异常值undefined或null按无效值处理。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| type | 'onDetach' | Yes | 取消订阅CanvasRenderingContext2D与Canvas组件解除绑定的回调。  异常值undefined或null按无效值处理。 |
| callback | Callback&lt;void> | No |  |
...
订阅CanvasRenderingContext2D与Canvas组件发生绑定的场景。 > **说明：** > > CanvasRenderingContext2D对象在同一时间只能与一个Canvas组件绑定。 > 当CanvasRenderingContext2D对象和Canvas组件发生绑定时，会触发'onAttach'回调， > 表示可以获取到[canvas](#canvas13)。 > 避免在'onAttach'中执行绘制方法，应保证Canvas组件已经 > [onReady](docroot://reference/apis-arkui/arkui-ts/ts-components-canvas-canvas.md) > 再进行绘制。 > 触发'onAttach'回调的一般场景： > 1、Canvas组件创建时绑定CanvasRenderingContext2D对象； > 2、CanvasRenderingContext2D对象新绑定一个Canvas组件时。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| type | 'onAttach' | Yes | 订阅CanvasRenderingContext2D与Canvas组件发生绑定的回调。  异常值undefined或null按无效值处理。 |
| callback | Callback&lt;void> | Yes | 订阅CanvasRenderingContext2D与Canvas组件  发生绑定后触发的回调。  异常值undefined或null按无效值处理。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| type | 'onDetach' | Yes | 订阅CanvasRenderingContext2D与Canvas组件解除绑定的回调。  异常值undefined或null按无效值处理。 |
| callback | Callback&lt;void> | Yes | 订阅CanvasRenderingContext2D与Canvas组件  解除绑定后触发的回调。  异常值undefined或null按无效值处理。 |
...
配置并启动AI分析功能，使用Promise异步回调。使用前需先设置 [enableAnalyzer](docroot://reference/apis-arkui/arkui-ts/ts-components-canvas-canvas.md#enableanalyzer12) 为true，启用图像AI分析能力。 该方法调用时，将截取调用时刻的画面帧进行分析，使用时需注意启动分析的时机， 避免出现画面和分析内容不一致的情况。 未执行完重复调用该方法会触发错误回调。 > **说明：** > > 分析类型不支持动态修改。 > 当检测到画面有变化时，分析结果将自动销毁，可重新调用本接口启动分析。 > 该特性依赖设备能力，不支持该能力的情况下，将返回错误码。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| config | ImageAnalyzerConfig | Yes | 执行AI分析所需要的入参，用于配置AI分析功能。  异常值undefined或null按无效值处理。 |
