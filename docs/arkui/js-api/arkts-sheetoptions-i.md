...
继承自[BindOptions](arkts-bindoptions-i.md#BindOptions)。 半模态页面内容选项。

**Inheritance:** SheetOptionsextends: BindOptions.
...
设置半模态面板内容区边缘回弹效果，支持单边生效。 **默认值**：默认双边生效，即[EffectEdge](arkts-effectedge-e.md#EffectEdge).START | [EffectEdge](arkts-effectedge-e.md#EffectEdge).END（即数值3）。 **说明：** 1. 仅上边缘生效：[EffectEdge](arkts-effectedge-e.md#EffectEdge).START。 2. 仅下边缘生效：[EffectEdge](arkts-effectedge-e.md#EffectEdge).END。 3. 双边生效：[EffectEdge](arkts-effectedge-e.md#EffectEdge).START | [EffectEdge](arkts-effectedge-e.md#EffectEdge).END（即数值3）。 4. 双边不生效：[EffectEdge](arkts-effectedge-e.md#EffectEdge).START & [EffectEdge](arkts-effectedge-e.md#EffectEdge).END（即数值0）。
...
设置半模态弹窗偏移量。当半模态为底部弹窗时，支持设置底部间距。不支持设置半模态的[SheetOptions](arkts-sheetoptions-i.md#SheetOptions)中的detents属性。y轴设置为负数的时候不生效。 默认值：x轴为0vp，y轴坐标为0vp。 **系统接口：** 此接口为系统接口。
...
半模态页面交互式关闭回调函数。 **说明：** 当用户执行下拉关闭、侧拉关闭、点击遮罩层关闭、点击关闭按钮的交互操作时，如果已注册回调函数，模态窗口将不会立即关闭。要关闭半模态，需在回调函数中调用shouldDismiss.dismiss()方法来实现。 如果不注册该回调函数，则用户执行下拉关闭、侧拉关闭、点击遮罩层关闭、点击关闭按钮的交互操作时，正常关闭半模态，无其他行为。 侧拉关闭又包含侧滑（左滑/右滑）、三键back、键盘ESC关闭。 建议在[二次确认](docroot://ui/arkts-sheet-page.md#二次确认能力)场景使用。
...
在UIContext实例对应的窗口中显示半模态。 **说明：** 使用[openBindSheet](docroot://reference/apis-arkui/arkts-apis-uicontext-uicontext.md#openbindsheet12)启动的半模态页面，不支持设置、更 新该属性。
...
设置组件的系统材质。 默认值：undefined，会清除由该接口设置的材质效果。 **说明**: 不同系统材质对应不同的属性影响效果，该接口影响背景色 [backgroundColor](docroot://reference/apis-arkui/arkui-ts/ts-universal-attributes-background.md#backgroundcolor)、边框 颜色[borderColor](arkts-commonmethod-c.md#borderColor)、边框宽度[borderWidth](arkts-commonmethod-c.md#borderWidth)、阴影 [shadow](arkts-commonmethod-c.md#shadow)，不建议与上述接口一起使用。使用示例请参考 [示例10（半模态设置系统材质）](docroot://reference/apis-arkui/arkui-ts/ts-universal-attributes-sheet-transition.md#示例10半模态设置系统材质)。
...
控制条是否悬浮显示，true为悬浮显示，false为不悬浮显示。 默认值：false **说明：** 悬浮效果只在控制条显示的场景生效，且控制条不占位。 title传入[CustomBuilder](docroot://reference/apis-arkui/arkui-ts/ts-types.md#custombuilder8)时enableFloatingDragBar始终为 false。
