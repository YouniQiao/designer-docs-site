...
当前，MultiNavigation的路由栈仅支持由使用方自行创建，不支持通过回调方式获取。请勿使用[NavDestination]nav_destination的 [onReady]NavDestinationAttribute#onReady等类似事件或接口来获取NavPathStack并进行栈操作，因为这可能会导致不可预知的问题。

**Inheritance:** MultiNavPathStackextends: NavPathStack.
...
清除栈中所有页面。 > **说明：** > 当调用[keepBottomPage](arkts-multinavpathstack-c.md#keepBottomPage)接口并设置为true时，会保留栈底页面。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| disable | boolean | Yes | 是否关闭转场动画。 默认值：false true：关闭转场动画。 false：不关闭转场动画。 |
...
| Type | Description |
| --- | --- |
| Array&lt;number> | Indexes of all the matching navigation destination pages.  Value range of the number type: [0, +∞). |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| index | number | Yes | NavDestination页面的位置索引。 取值范围：[0, +∞) |

**Return value:**

| Type | Description |
| --- | --- |
| Object | Object: parameter information of the matching navigation destination page.  undefined: returned when an invalid index is provided. |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| keepBottom | boolean | Yes | 是否保留栈底页面。 默认值：false true：保留栈底页面。 false：不保留栈底页面。 |
...
弹出路由栈栈顶元素。 > **说明：** > 当调用[keepBottomPage](arkts-multinavpathstack-c.md#keepBottomPage)接口并设置为true时，会保留栈底页面。
...
弹出路由栈栈顶元素，并触发onPop回调传入页面处理结果。 > **说明：** > 当调用[keepBottomPage](arkts-multinavpathstack-c.md#keepBottomPage)接口并设置为true时，会保留栈底页面。
...
| Type | Description |
| --- | --- |
| number | Returns the index of the first navigation destination page that matches name from the  bottom of the navigation stack; returns -1 if no such a page is found.  Value range: [-1, +∞). |
...
| name | string | Yes | NavDestination页面名称。 |
| param | Object | Yes | NavDestination页面详细参数。 |
| onPop | base.Callback&lt;PopInfo> | No |  |
| animated | boolean | No |  |
| policy | SplitPolicy | No |  |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| indexes | Array&lt;number> | Yes | 待删除NavDestination页面的索引值数组。 number类型的取值范围：[0, +∞) |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| minPercent | number | Yes | 最小主页宽度百分比。 取值范围：[0, 100] |
| maxPercent | number | Yes | 最大主页宽度百分比。 取值范围：[0, 100] |
