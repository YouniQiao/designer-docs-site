...
| --- | --- | --- | --- |
| content | string \| Resource | No | 文本内容。当不包含子组件[Span]{@link span}和未设置[属性字符串]{@link styled_string}时该参数生效。 默认  值：' ' 说明： 显示内容的优先级：属性字符串>Span>Text的文本内容。 |
| value | TextOptions | No | 文本组件初始化选项。 [since 11] |
