...
frameNode中[getEvent('Grid')]FrameNode:typeNode.getEvent(node: FrameNode, nodeType: 'Grid')方法的返回值，可用于给Grid节点设置 滚动事件。 UIGridEvent继承于[UIScrollableCommonEvent]UIScrollableCommonEvent。

**Inheritance:** UIGridEventextends: UIScrollableCommonEvent.
...
设置[onScrollIndex](docroot://reference/apis-arkui/arkui-ts/ts-container-grid.md#onscrollindex)事件的回调。 方法入参为undefined时，会重置事件回调。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| callback | OnGridScrollIndexCallback \| undefined | Yes | onScrollIndex事件的回调函数。 |
