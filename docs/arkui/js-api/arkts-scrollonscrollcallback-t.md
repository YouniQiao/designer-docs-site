...
| --- | --- | --- | --- |
| xOffset | number | Yes | 相对于上一帧水平方向的偏移量，Scroll中的内容向左滚动时偏移量为正，向右滚动时偏移量为负。  单位vp |
| yOffset | number | Yes | 相对于上一帧竖直方向的偏移量，Scroll中的内容向上滚动时偏移量为正，向下滚动时偏移量为负。  单位vp |
| scrollState | ScrollState | Yes | 当前滚动状态。 |
