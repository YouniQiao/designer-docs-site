...
| --- | --- | --- | --- |
| value | boolean | Yes | 当系统配置发生变化时，是否自动刷新颜色值。  如果设置为true，则使用ColorMetrics.resourceColor()创建的对象的颜色值  会在系统配置发生变化时自动更新。如果设置为false，则对象的颜色值  由ColorMetrics.resourceColor()创建的，不会自动更新。  默认值为false。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| overlayColor | ColorMetrics | Yes | 要叠加在上方的颜色对象。alpha属性决定叠加强度。1.0表示完全覆盖，0.0表示完全透明，混合结果为原色。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| colorSpace | ColorSpace | Yes | 颜色空间，用于指定颜色的色彩空间。使用ColorSpace.DISPLAY_P3，需要对应窗口调用  [setWindowColorSpace](docroot://reference/apis-arkui/arkts-apis-window-Window.md#setwindowcolorspace9-1)接口，将当前窗  口设置为广色域模式。 |
| red | number | Yes | 颜色的R分量（红色），值是0~1的浮动数值。 |
| green | number | Yes | 颜色的G分量（绿色），值是0~1的浮动数值。 |
| blue | number | Yes | 颜色的B分量（蓝色），值是0~1的浮动数值。 |
| alpha | number | No |  |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| colorSpace | ColorSpace | Yes | 颜色的颜色空间。  支持SRGB、Display_P3、BT2020颜色空间。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| linearExposure | double | Yes | 曝光值中的线性曝光值。  值为1.0表示标准曝光。  大于1.0的值会线性增加亮度。 |
| colorSpace | ColorSpace | Yes | 颜色的颜色空间。  支持SRGB、Display_P3、BT2020颜色空间。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| value | number | Yes | HEX格式颜色。 取值范围：支持rgb或者argb |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| color | ResourceColor | Yes | 资源格式颜色。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| red | number | Yes | 颜色的R分量（红色），值是0~255的整数。 |
| green | number | Yes | 颜色的G分量（绿色），值是0~255的整数。 |
| blue | number | Yes | 颜色的B分量（蓝色），值是0~255的整数。 |
| alpha | number | No | 颜色的A分量（透明度），值是0.0~1.0的浮点数，默认值为1.0，不透明。 说明： alpha小于0为全透明，大于1为不透明。 |
