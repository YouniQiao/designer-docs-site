...
| --- | --- | --- | --- |
| eventMask | int | Yes | 事件类型掩码，指定要监视的事件类型  位运算。  取值限定为整数。 |
| listener | InputEventListener | Yes | 事件监听器回调函数。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| value | AnimateParam | Yes | 设置动画效果相关参数。 |
| event | () => void | Yes | 指定显示动效的闭包函数，在闭包函数中导致的状态变化系统会自动插入过渡动画。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| param | AnimateParam | Yes | 设置动画效果相关参数。 |
| processor | Callback&lt;void> | Yes | 回调函数。指定显示动效的闭包函数，在闭包函数中导致的状态变化系统会自动插入过渡动画。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| bindSheetContent | ComponentContent&lt;T> | Yes | 半模态页面中显示的组件内容。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| context | common.UIAbilityContext \| common.ExtensionContext | Yes | [UIAbility]@ohos.app.ability.UIAbility或  [ExtensionAbility](../../apis-ability-kit/arkts-apis/arkts-extensionability-c.md#ExtensionAbility)所对应的上下文环境。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enabled | boolean | Yes | 启用或禁用事件透传。默认值为false。 |
| eventType | RawInputEventType | Yes | 原始输入事件的类型。 |
...
| --- | --- | --- | --- |
| id | string | Yes | 组件的id。 |
| isFrozen | boolean | Yes | 是否设置冻结。 true表示设置冻结，false表示设置不冻结。 默认值为false。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| uniqueId | number | Yes | 组件的uniqueId。 |
| isFrozen | boolean | Yes | 是否设置冻结。 true表示设置冻结，false表示设置不冻结。 默认值为false。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| id | string | Yes | 节点对应的[组件标识]common。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| param | KeyframeAnimateParam | Yes | 关键帧动画的整体动画参数。 |
| keyframes | Array&lt;KeyframeState> | Yes | 所有的关键帧状态的列表。 |
...
| --- | --- | --- | --- |
| bindSheetContent | ComponentContent&lt;T> | Yes | 半模态页面中显示的组件内容。 |
| sheetOptions | SheetOptions | No | 半模态页面样式。 说明： 1. 不支持设置SheetOptions.uiContext，该属性的值固定为当前实例的  UIContext。 2. 若不传递targetId，则不支持设置SheetOptions.preferType为POPUP样式，若设置了POPUP样式则使用CENTER样式替代。 3. 若不传递  targetId，则不支持设置SheetOptions.mode为EMBEDDED模式，默认为OVERLAY模式。 4. 其余属性的默认值参考[SheetOptions]SheetOptions文  档。 |
| targetId | number | No | 需要绑定组件的ID，若不指定则不绑定任何组件。id不存在时返回错误码120004。在传入undefined时返回错误码401。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| frameCallback | FrameCallback | Yes | 下一帧需要执行的回调。 |
| delayTime | number | Yes | 延迟的时间，以毫秒为单位。传入null、undefined或小于0的值，会按0处理。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| enabled | boolean | Yes | 使能开关项：true开启，false关闭；  默认不开启，由系统应用按需开启。  默认值：false  默认值：false  默认值：false  默认值：false  配置为异常undefined时，恢复为默认值false |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| monitor | InputEventMonitor | Yes | 监控标识对象（由addLocalInputEventMonitor返回）。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| feature | CustomKeyboardContinueFeature | Yes | 自定义键盘接续特性。 |
...
| --- | --- | --- | --- |
| id | string | Yes | 组件id。 |
| value | number | Yes | 组件压暗程度取值范围[0,1], 由0到1逐渐变亮。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| value | number | Yes | 内存中缓存解码后图片的数量上限 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| value | KeyboardAvoidMode | Yes | 配置虚拟键盘抬起时页面的避让模式。 默认值：KeyboardAvoidMode.OFFSET，键盘抬起时默认避让模式为上抬。setKeyboardAvoidMode传入异常值时，该属性设置不生效。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| mode | PixelRoundMode | Yes | 像素取整模式。 默认值：PixelRoundMode.PIXEL_ROUND_ON_LAYOUT_FINISH 设置异常值时，该属性为默认值。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| count | number | Yes | HSP资源管理器的缓存限制必须为非负整数。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| policy | TextSelectionClearPolicy | Yes | 文本选择清除策略。 |
...
| --- | --- | --- | --- |
| bindSheetContent | ComponentContent&lt;T> | Yes | 半模态页面中显示的组件内容。 |
| sheetOptions | SheetOptions | Yes | 半模态页面样式。 说明： 不支持更新SheetOptions.uiContext、SheetOptions.mode、回调函  数。 |
| partialUpdate | boolean | No | 半模态页面更新方式, 默认值为false。 说明： 1. true为增量更新，保留当前值，更新SheetOptions中的指定属性。  2. false为全量更新，除SheetOptions中的指定属性，其他属性恢复默认值。 |
