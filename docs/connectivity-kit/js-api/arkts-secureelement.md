...
| Name | Description |
| --- | --- |
| [createService](arkts-connectivity-createservice-f.md#createservice-1) | Establish a new connection that can be used to connect to all the SEs available in the system.The connection process can be quite long, so it happens in an asynchronous way. It is usable onlyif isConnected() returns true. |
| [newSEService](arkts-connectivity-newseservice-f.md#newseservice-1) | Establish a new connection that can be used to connect to all the SEs available in the system.The connection process can be quite long, so it happens in an asynchronous way. It is usable onlyif the specified callback is called or if isConnected() returns true. |
...
| [Session](arkts-connectivity-session-i.md) | Session represent a connection session to one of the SEs available on the device. These objectscan be used to get a communication channel with an applet in the SE. This channel can be the basic channelor a logical channel. |
