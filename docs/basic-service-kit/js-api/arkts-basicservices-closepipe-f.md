...
| --- | --- | --- | --- |
| pipe | USBDevicePipe | Yes | USB device pipe, which is used to determine the message control channel. You need tocall [usbManager.connectDevice](arkts-basicservices-connectdevice-f.md#connectdevice-1) to obtain its value. |

**Return value:**

| Type | Description |
| --- | --- |
| number | Returns **0** if the USB device pipe is closed successfully; returns an error code otherwise. The errorcodes are as follows:- 22: The service is abnormal. |
