...
| dest | ArrayBuffer | Yes | 目标缓冲区。 |
| source | ArrayBuffer | Yes | 源数据缓冲区。 |
| level | CompressLevel | Yes | 参考[CompressLevel枚举定义]zlib.CompressLevel。 |
| sourceLen | long | No | 源数据长度。默认值为0。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| flush | CompressFlushMode | Yes | 参考[CompressFlushMode定义]zlib.CompressFlushMode。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| source | Zip | Yes | 参考[Zip定义]zlib.Zip。 |
...
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| level | CompressLevel | Yes | 参考[CompressLevel枚举定义]zlib.CompressLevel。 |
| method | CompressMethod | Yes | 参考[CompressMethod枚举定义]zlib.CompressMethod。 |
| windowBits | int | Yes | 控制内存窗口的大小，并指定数据的格式（zlib、gzip、raw deflate）。取值如下： zlib格式：[1, 15]。 gzip格式：大于15  。 raw deflate格式：[-15, -1]。 |
| memLevel | MemLevel | Yes | 参考[MemLevel枚举定义]zlib.MemLevel。 |
| strategy | CompressStrategy | Yes | 参考[CompressStrategy枚举定义]zlib.CompressStrategy。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| bits | int | Yes | 要插入的位数，取值范围在0~16。 |
| value | int | Yes | 与位数相对应的位值。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| head | GzHeader | Yes | 从压缩数据流中提取的gzip头信息。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| goodLength | int | Yes | 匹配的长度阈值。 |
| maxLazy | int | Yes | 压缩算法在构建哈夫曼树时的延迟匹配策略，取值范围为0到4的整数。1到4，值越大，算法越‘懒’，匹配过程越慢，但可能生成更优的压缩结果。0：禁用懒惰匹配，  算法会尽快构建哈夫曼树，压缩速度快，但压缩率低。 |
| niceLength | int | Yes | 适合的延迟长度阈值。 |
| maxChain | int | Yes | 最大链条长度。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| backIn | InflateBackInputCallback | Yes | 一种函数，用于从末尾解压缩数据，以从输入源读取原始压缩数据。 |
| inDesc | object | Yes | 通用对象。 |
| backOut | InflateBackOutputCallback | Yes | 将解压缩的数据写入目标输出。 |
...
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| backIn | InflateBackInputCallback | Yes | 一种函数，用于从末尾解压缩数据，以从输入源读取原始压缩数据。 |
| inDesc | RecordData | Yes | 通用对象。 |
| backOut | InflateBackOutputCallback | Yes | 将解压缩的数据写入目标输出。 |
| outDesc | RecordData | Yes | 通用对象。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| windowBits | long | Yes | 控制内存窗口的大小，并指定数据的格式（zlib、gzip、raw deflate）。取值如下： zlib格式：[1, 15]。 gzip格式：大于1  5。 raw deflate格式：[-15, -1]。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| header | GzHeader | Yes | 从压缩数据流中提取的gzip头信息。 |
...
| --- | --- | --- | --- |
| strm | ZStream | Yes | 参考[ZStream定义]zlib.ZStream。 |
| bits | int | Yes | 指定要写入比特缓冲区的比特数。 |
| value | int | Yes | 用于填充比特缓冲区的比特值。 |
