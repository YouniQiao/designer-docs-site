...
Note: When [request.agent.search](arkts-basicservices-agent-search-f.md#search-2)is used for query, this value must be within the range of [after,before] for the task ID to be obtained. For details about **before** and **after**, see [Filter](arkts-basicservices-agent-filter-i.md).
...
- Use [request.agent.show](arkts-basicservices-agent-show-f.md#show-2)and [request.agent.touch](arkts-basicservices-agent-touch-f.md#touch-2)to query the value.
