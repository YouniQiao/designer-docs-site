...
Implements a macro video session, which sets the parameters of the macro video mode and saves all [CameraInput]camera.CameraInput and [CameraOutput]camera.CameraOutput instances required to run the camera. It inherits from [Session]camera.Session.

**Inheritance:** MacroVideoSessionextends: Session, Flash, AutoExposure, Focus, Zoom, ColorEffect, ManualFocus, ColorManagement, Notification.
