...
| [TRUNC](arkts-corefile-openmode-con.md#trunc) | If the file exists and is opened in write-only or read/write mode, truncate the filelength to 0. |
...
| [NONBLOCK](arkts-corefile-openmode-con.md#nonblock) | If **path** points to a named pipe (FIFO), block special file, or character specialfile, perform non-blocking operations on the open file and in subsequent I/Os. |
