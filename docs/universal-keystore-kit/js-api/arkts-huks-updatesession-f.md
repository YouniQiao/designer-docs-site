...
| options | HuksOptions | Yes | updateSession的参数集合。 |
| callback | AsyncCallback&lt;HuksReturnResult> | Yes | 回调函数。当密钥操作update成功时，err为undefined，data为获取到的HuksReturnResult；否  则为错误对象。 |
...
| --- | --- | --- | --- |
| handle | long | Yes | updateSession操作的uint64类型的handle值。 |
| options | HuksOptions | Yes | updateSession操作的参数集合。 |
