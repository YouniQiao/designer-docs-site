...
| srcRect | common2D.Rect | Yes | pixelMap的待绘制区域。  图片最左侧和最上侧对应位置0，最右侧和最下侧对应位置1。right需大于left，bottom需大于top。 |
| dstRect | common2D.Rect | Yes | pixelMap在mask挂载的节点上的绘制区域。  节点最左侧和最上侧对应位置0，最右侧和最下侧对应位置1。right需大于left，bottom需大于top。 |
| fillColor | Color | No |  |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| center | common2D.Point | Yes | 设置椭圆的中心点，[0, 0]为组件左上角，[1, 1]为组件的右下角。  取值范围[-10, 10]，可取浮点数，超出边界会在实现时自动截断。 |
| radiusX | double | Yes | 设置椭圆的长轴，半径为1等于组件的高度。  取值范围[0, 10]，可取浮点数，超出边界会在实现时自动截断。 |
| radiusY | double | Yes | 设置椭圆的短轴，半径为1等于组件的高度。  取值范围[0, 10]，可取浮点数，超出边界会在实现时自动截断。 |
| gradients | Array&lt;[double, double]> | Yes | 数组中保存的二元数组表示梯度：[RGBA颜色, 位置]。  RGBA颜色四通道使用相同的值，可看作一个灰度值；位置表示沿径向方向向外时RGBA颜色对应的分布位置；  RGBA颜色与位置的取值范围均为[0, 1]，可取浮点数，小于0的转为0，大于1的转为1。  位置参数值须严格递增，Array数组中二元数组个数必须大于等于2，二元数组中的元素不能为空，否则该椭圆分布效果不生效。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| center | common2D.Point | Yes | 设置波环圆心在组件上的位置，[0, 0]为组件左上角，[1, 1]为组件的右下角。  取值范围[-10, 10]，超出边界会在实现时自动截断。 |
| radius | double | Yes | 设置波环的半径，半径为1等于组件的高度。  取值范围[0, 10]，超出边界会在实现时自动截断。 |
| width | double | Yes | 设置波环的宽度。  取值范围[0, 10]，超出边界会在实现时自动截断。 |
| offset | double | No |  |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| center | common2D.Point | Yes | 设置单波波源的中心点，[0, 0]为组件左上角，[1, 1]为组件的右下角。  取值范围[-10, 10]，可取浮点数，超出边界会在实现时自动截断。 |
| width | double | Yes | 设置单波圆环的宽度。  取值范围[0, 5]，可取浮点数，超出边界会在实现时自动截断。 |
| propagationRadius | double | Yes | 设置单波圆环的扩散外径。  取值范围[0, 10]，可取浮点数，超出边界会在实现时自动截断。 |
| blurRadius | double | Yes | 设置单波圆环的模糊外径，模糊半径为0则是实边圆环，否则是虚边圆环。  取值范围[0, 5]，可取浮点数，超出边界会在实现时自动截断。 |
| turbulenceStrength | double | No |  |
