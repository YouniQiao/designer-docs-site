...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| total | int | Yes | Maximum number of results to query. The value must be a non-negative integer. If the value is  a negative number, the entire result set is queried. |
| offset | int | Yes | Start position of the query result. The value must be a non-negative integer. If the value is  a negative number, the entire result set is queried. If offset exceeds the end of the result set, the query  result is empty. |
