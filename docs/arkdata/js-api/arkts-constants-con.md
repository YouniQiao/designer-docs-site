# Constants

## MAX_KEY_LENGTH

```TypeScript
const MAX_KEY_LENGTH = 1024
```

max key length. Maximum length of a key in the KV store, in bytes.

**Since:** 7

**Deprecated since:** 9

**Substitute:** ohos.data.distributedKVStore.Constants#MAX_KEY_LENGTH

**System capability:** SystemCapability.DistributedDataManager.KVStore.Core

## MAX_VALUE_LENGTH

```TypeScript
const MAX_VALUE_LENGTH = 4194303
```

max value length. Maximum length of a value in the KV store, in bytes.

**Since:** 7

**Deprecated since:** 9

**Substitute:** ohos.data.distributedKVStore.Constants#MAX_VALUE_LENGTH

**System capability:** SystemCapability.DistributedDataManager.KVStore.Core

## MAX_KEY_LENGTH_DEVICE

```TypeScript
const MAX_KEY_LENGTH_DEVICE = 896
```

max device coordinate key length. Maximum length of a device key, in bytes.

**Since:** 7

**Deprecated since:** 9

**Substitute:** ohos.data.distributedKVStore.Constants#MAX_KEY_LENGTH_DEVICEs

**System capability:** SystemCapability.DistributedDataManager.KVStore.Core

## MAX_STORE_ID_LENGTH

```TypeScript
const MAX_STORE_ID_LENGTH = 128
```

max store id length. Maximum length of a KV store ID, in bytes.

**Since:** 7

**Deprecated since:** 9

**Substitute:** ohos.data.distributedKVStore.Constants#MAX_STORE_ID_LENGTH

**System capability:** SystemCapability.DistributedDataManager.KVStore.Core

## MAX_QUERY_LENGTH

```TypeScript
const MAX_QUERY_LENGTH = 512000
```

max query length. Maximum query length, in bytes.

**Since:** 7

**Deprecated since:** 9

**Substitute:** ohos.data.distributedKVStore.Constants#MAX_QUERY_LENGTH

**System capability:** SystemCapability.DistributedDataManager.KVStore.Core

## MAX_BATCH_SIZE

```TypeScript
const MAX_BATCH_SIZE = 128
```

max batch operation size. Maximum number of batch operations.

**Since:** 7

**Deprecated since:** 9

**Substitute:** ohos.data.distributedKVStore.Constants#MAX_BATCH_SIZE

**System capability:** SystemCapability.DistributedDataManager.KVStore.Core

