...
Verify类，使用Verify方法之前需要创建该类的实例进行操作，通过 [createVerify(algName: string): Verify]cryptoFramework.createVerify方法构造此实例。按序调用本类中的init、update、 verify方法完成签名操作。验签操作的示例代码详见 [签名验签开发指导](docroot://security/CryptoArchitectureKit/crypto-rsa-sign-sig-verify-pkcs1.md)。 Verify类不支持重复初始化，当业务方需要使用新密钥验签时，需要重新创建新Verify对象并调用init初始化。 业务方使用时，在createVerify时确定验签的模式，调用init接口设置密钥。 当被签名的消息较短时，可在init初始化后，（无需update）直接调用verify接口传入被签名的消息和签名（signatureData）进行验签。 当被签名的消息较长时，可通过update接口分段传入被签名的消息，最后调用verify接口对消息全文进行验签。verify接口的data入参在API 10之前只 支持DataBlob， API 10之后增加支持null。业务方可在循环中调用update接口，循环结束后调用verify传入签名（signatureData）进行验签。 当使用DSA算法进行验签，并设置了摘要算法为NoHash时，则不支持update操作，update接口会返回错误码ERR_CRYPTO_OPERATION。
...
| --- | --- | --- | --- |
| itemType | SignSpecItem | Yes | 用于指定需要获取的验签参数。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| pubKey | PubKey | Yes | 公钥对象，用于Verify的初始化。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当验签初始化成功，err为undefined，否则为错误对象。 |
...
| 17630001 | 密码操作错误。 |
| 17620003 | 参数检查失败。可能的原因：  1. 密钥类型不正确。 [since 26.0.0] |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| signatureData | DataBlob | Yes | 签名数据。 |
...
设置验签参数。常用的验签参数直接通过[createVerify]cryptoFramework.createVerify 来指定，剩余参数通过本接口指定。 支持RSA算法和SM2算法，从API version 11开始，支持SM2算法设置签名验证参数。 验签的参数应当与签名的参数保持一致。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| itemType | SignSpecItem | Yes | 用于指定需要设置的验签参数。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| itemType | SignSpecItem | Yes | 用于指定需要设置的验签参数类型。 |
| itemValue | int \| Uint8Array | Yes | 指定验签参数的具体值。 |
...
| 801 | 该操作不支持。 |
| 17620001 | 内存操作失败。 |
| 17620002 | 获取Native对象失败或参数转换失败。 [since 26.0.0] |
| 17620003 | 参数检查失败。 [since 26.0.0] |
| 17620004 | 无效的函数调用。 [since 26.0.0] |
| 17630001 | 密码操作错误。 |
...
为Verify对象设置指定参数。 目前仅支持ML-DSA算法中的ML_DSA_DETERMINISTIC和ML_DSA_MU参数。ML_DSA_CONTEXT参数请使用 [setVerifySpec()]cryptoFramework.Verify.setVerifySpec(itemType: SignSpecItem, itemValue: int | Uint8Array)。 验签的参数应当与签名的参数保持一致。
...
追加待验签数据，使用callback异步回调完成更新。 必须在对[Verify]cryptoFramework.Verify实例使用[init]cryptoFramework.Verify.init或 [initSync]cryptoFramework.Verify.initSync初始化后，才能使用本函数。 > **说明：** > > 根据数据量，可以不调用update（即[init]cryptoFramework.Verify.init > 完成后直接调用 > [verify]cryptoFramework.Verify.verify(data: DataBlob | null, signatureData: DataBlob, callback: AsyncCallback<boolean>) > ）或多次调用update。 > > 算法库目前没有对update（单次或累计）的数据量设置大小限制，建议对于大数据量的验签操作，采用多次update的方式传入数据，避免一次性申请 > 过大内存。 > > 验签使用多次update操作的示例代码详见 > [使用RSA密钥对分段签名验签](docroot://security/CryptoArchitectureKit/crypto-rsa-sign-sig-verify-pkcs1-by-segment.md) > ，其余算法操作类似。 > > OnlyVerify模式下，不支持update操作，直接使用verify传入数据即可。 > > 当使用DSA算法进行验签，并设置了摘要算法为NoHash时，则不支持update操作，update接口会返回错误码ERR_CRYPTO_OPERATION。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| data | DataBlob | Yes | 传入的消息。 |
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当验签更新成功，err为undefined，否则为错误对象。 |
...
追加待验签数据，使用Promise异步回调完成更新。 必须在对[Verify]cryptoFramework.Verify实例使用[init()]cryptoFramework.Verify.init初始化后，才能使 用本函数。 > **说明：** > > 根据数据量，可以不调用update（即[init]cryptoFramework.Verify.init完成后直接调用 > [verify]cryptoFramework.Verify.verify(data: DataBlob | null, signatureData: DataBlob)）或多次调用update。 > > 算法库目前没有对update（单次或累计）的数据量设置大小限制，建议对于大数据量的验签操作，采用多次update的方式传入数据，避免一次性申请 > 过大内存。 > > 验签使用多次update操作的示例代码详见 > [使用RSA密钥对分段签名验签](docroot://security/CryptoArchitectureKit/crypto-rsa-sign-sig-verify-pkcs1-by-segment.md) > ，其余算法操作类似。 > > OnlyVerify模式下，不支持update操作，直接使用verify传入数据即可。 > > 当使用DSA算法进行验签，并设置了摘要算法为NoHash时，则不支持update操作，update接口会返回错误码ERR_CRYPTO_OPERATION。
...
追加待验签数据，通过同步方式完成更新。 必须在对[Verify]cryptoFramework.Verify实例使用[initSync()]cryptoFramework.Verify.initSync初始化后，才 能使用本函数。 > **说明：** > > 根据数据量，可以不调用updateSync（即[initSync]cryptoFramework.Verify.initSync完成后直接调用 > [verifySync]cryptoFramework.Verify.verifySync）或多次调用updateSync。 > > 算法库目前没有对updateSync（单次或累计）的数据量设置大小限制，建议对于大数据量的验签操作，采用多次updateSync的方式传入数据，避免 > 一次性申请过大内存。 > > 验签使用多次updateSync操作的示例代码详见 > [使用RSA密钥对分段签名验签](docroot://security/CryptoArchitectureKit/crypto-rsa-sign-sig-verify-pkcs1-by-segment.md)， > 其余算法操作类似。 > > OnlyVerify模式下，不支持updateSync操作，需要直接使用verifySync传入数据。 > > 当使用DSA算法进行验签，并设置了摘要算法为NoHash时，则不支持updateSync操作，updateSync接口会返回错误码ERR_CRYPTO_OPERATION。 <br><br>**说明：** <br>建议优先使用异步API{@link update}。同步API可能因系统繁忙、高负载等原因耗时较长而阻塞主线程。 因此建议在子线程中调用同步API，以避免阻塞主线程。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| data | DataBlob | Yes | 待验签的数据。 |
| signatureData | DataBlob | Yes | 签名数据。 |
| callback | AsyncCallback&lt;boolean> | Yes | 回调函数。返回true表示验签通过；返回false表示验签失败。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| data | DataBlob \| null | Yes | 传入的消息。API 10之前只支持DataBlob， API 10之后增加支持null。 |
| signatureData | DataBlob | Yes | 签名数据。 |
| callback | AsyncCallback&lt;boolean> | Yes | 回调函数。返回true表示验签通过；返回false表示验签不通过。 |
