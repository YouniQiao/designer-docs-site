...
Provides APIs for private key operations. **PriKey** is a child class of [Key](arkts-cryptoarchitecture-key-i.md). It needs to be passed in during asymmetric decryption, signing, and key agreement. The private key can be generated by using the asymmetric key generator [AsyKeyGenerator](arkts-cryptoarchitecture-asykeygenerator-i.md) or [AsyKeyGeneratorBySpec](arkts-cryptoarchitecture-asykeygeneratorbyspec-i.md).
