...
私钥，是[Key]cryptoFramework.Key的子类，在非对称解密、签名、密钥协商时需要将其作为输入使用。 私钥可以通过非对称密钥生成器[AsyKeyGenerator]cryptoFramework.AsyKeyGenerator、 [AsyKeyGeneratorBySpec]cryptoFramework.AsyKeyGeneratorBySpec来生成。

**Inheritance:** PriKeyextends: Key.
...
| --- | --- | --- | --- |
| itemType | AsyKeySpecItem | Yes | 指定的密钥参数类型。 |
...
支持根据指定的密钥格式（如采用哪个规范），获取满足ASN.1语法、DER编码的私钥数据。 > **说明：** > > 本接口和[Key.getEncoded()]cryptoFramework.Key.getEncoded的区别是： > 1. 本接口可以指定获取密钥数据的格式。 > 2. [Key.getEncoded()]cryptoFramework.Key.getEncoded不支持指定获取密钥数据的格式。
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| format | string | Yes | 用于指定当前密钥格式。支持EC密钥，format取值支持"PKCS8"。  从API版本26.0.0开始，支持RSA密钥，format取值支持"PKCS1"和"PKCS8"。  从API版本26.0.0开始，支持ML-DSA和ML-KEM密钥，format取值支持"PKCS8"。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| format | string | Yes | 指定的获取密钥字符串的编码格式。支持RSA密钥，format取值支持'PKCS8'或'PKCS1'。  自API版本26.0.0起，支持EC密钥，format取值支持'PKCS8'或'EC'。  自API版本26.0.0起，支持ML-DSA和ML-KEM密钥，format取值支持'PKCS8'。 |
...
| config | KeyEncodingConfig | Yes | 指定编码的算法跟口令，对私钥进行编码操作。 |
