...
Random类，调用Random方法生成随机数。调用前，需要通过[createRandom]cryptoFramework.createRandom构造Random实例。
...
| --- | --- | --- | --- |
| len | int | Yes | 表示生成随机数的长度，单位为bytes，范围在[1, INT_MAX]。 |
| callback | AsyncCallback&lt;DataBlob> | Yes | 回调函数。当生成随机数成功时，err为undefined，data为获取到的随机数；否则为  错误对象。 |
