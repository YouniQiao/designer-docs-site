...
非对称密钥生成器。在使用该类的方法前，需要先使用[createAsyKeyGenerator]cryptoFramework.createAsyKeyGenerator方法构建 一个AsyKeyGenerator实例。
...
| --- | --- | --- | --- |
| pubKey | DataBlob | Yes | 公钥材料。 |
| priKey | DataBlob | Yes | 私钥材料。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| pubKey | DataBlob \| null | Yes | 指定的公钥材料。如果公钥不需要转换，请传入null。API 10之前只支持DataBlob， API 10之后增  加支持null。 |
| priKey | DataBlob \| null | Yes | 指定的私钥材料。如果私钥不需要转换，请传入null。API 10之前只支持DataBlob， API 10之后增  加支持null。 |
| callback | AsyncCallback&lt;KeyPair> | Yes | 回调函数。当生成非对称密钥成功时，err为undefined，data为获取到的KeyPair；否则  为错误对象。 |
...
| Name | Type | Mandatory | Description |
| --- | --- | --- | --- |
| pubKey | DataBlob \| null | Yes | 指定公钥材料。如果公钥无需转换，请传入null。API 10前仅支持DataBlob，API 10起支持传入  null。 |
| priKey | DataBlob \| null | Yes | 指定私钥材料。如果私钥无需转换，请传入null。API 10前仅支持DataBlob，API 10起支持传入  null。 |
...
| priKey | string \| null | Yes | 指定的私钥材料。如果私钥不需要转换，请传入null。 说明：公钥和私钥材料不能同时为null  或空字符串。 |
...
同步获取指定数据，生成非对称密钥。 > **说明：** > convertPemKeySync接口与convertPemKey接口注意事项相同，见 > [convertPemKey]cryptoFramework.AsyKeyGenerator.convertPemKey(pubKey: string | null, priKey: string | null) > 接口说明。 <br><br>**说明：** <br>建议优先使用异步API{@link convertPemKey}。同步API可能因系统繁忙、高负载等原因耗时较长而阻塞主线程。 因此建议在子线程中调用同步API，以避免阻塞主线程。
...
| --- | --- | --- | --- |
| pubKey | string \| null | Yes | 指定的公钥材料。如果公钥不需要转换，请传入null。 |
| priKey | string \| null | Yes | 指定私钥材料。私钥无需转换时，请传入null。 说明：公钥和私钥材料不能同时为null或  空字符串。 |
...
获取指定数据生成非对称密钥。支持加密的私钥，同步传入私钥口令解密私钥。使用同步方法。 > **说明：** > convertPemKeySync接口与convertPemKey接口注意事项相同，见 > [convertPemKey]cryptoFramework.AsyKeyGenerator.convertPemKey(pubKey: string | null, priKey: string | null, password: string) > 接口说明。 <br><br>**说明：** <br>建议优先使用异步API{@link convertPemKey}。同步API可能因系统繁忙、高负载等原因耗时较长而阻塞主线程。 因此建议在子线程中调用同步API，以避免阻塞主线程。
...
| --- | --- | --- | --- |
| pubKey | string \| null | Yes | 指定的公钥材料。如果公钥不需要转换，请传入null。 |
| priKey | string \| null | Yes | 指定私钥材料。若无需转换，请传入 null。注意：公钥与私钥材料不可同时为 null。 |
| password | string | Yes | 指定口令，用于解密私钥。 |
