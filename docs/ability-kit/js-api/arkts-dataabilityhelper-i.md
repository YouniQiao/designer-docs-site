...
可以通过[acquireDataAbilityHelper]./../@ohos.ability.featureAbility:featureAbility.acquireDataAbilityHelper接口获取 DataAbilityHelper对象。
...
| callback | AsyncCallback&lt;number> | Yes | 回调函数，返回插入的数据记录数。 |
...
| extras | PacMap | Yes | 表示扩展的键值对参数。 |
| callback | AsyncCallback&lt;PacMap> | Yes | 回调函数，返回扩展的键值对参数。 |
...
| callback | AsyncCallback&lt;number> | Yes | 回调函数，返回已删除的数据记录数。 |
...
| callback | AsyncCallback&lt;string> | Yes | 回调函数，返回反规范化uri对象。 |
...
| --- | --- | --- | --- |
| uri | string | Yes | 表示待处理的DataAbility。例：'dataability:///com.example.xxx.xxxx'。 |
| operations | Array&lt;DataAbilityOperation> | Yes | 表示数据操作数组，其中可以包含对数据库的多个不同操作。 |
| callback | AsyncCallback&lt;Array&lt;DataAbilityResult>> | Yes | 回调函数，在DataAbilityResult数组中返回每个操作的结果。 |
...
| callback | AsyncCallback&lt;Array&lt;string>> | Yes | 回调函数，返回匹配的媒体资源类型数组。 |
...
| callback | AsyncCallback&lt;string> | Yes | 回调函数，返回与uri指向数据匹配的媒体资源类型。 |
...
| callback | AsyncCallback&lt;number> | Yes | 回调函数，返回插入数据记录的索引。 |
...
| --- | --- | --- | --- |
| uri | string | Yes | 表示要规范化的uri对象。 |
| callback | AsyncCallback&lt;string> | Yes | 回调函数，如果数据功能支持uri规范化，则返回规范化uri对象；否则返回null。 |
...
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当通知注册的观察者成功，err为undefined，否则为错误对象。 |
...
| callback | AsyncCallback&lt;void> | Yes | 回调函数。当注册观察者以监听uri指定数据的数据变化通知成功，err为undefined，否则为错误对象。 |
...
| callback | AsyncCallback&lt;number> | Yes | 回调函数，返回文件描述符。 |
...
| callback | AsyncCallback&lt;ResultSet> | Yes | 回调函数，返回查询结果。 |
...
| predicates | dataAbility.DataAbilityPredicates | Yes | 表示筛选条件。当此参数为null时，应定义处理逻辑。 |
| callback | AsyncCallback&lt;number> | Yes | 回调函数，返回更新的数据记录数。 |
