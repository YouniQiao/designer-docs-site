# RemoteAbilityInfo (System API)

The module provides information about a remote ability. > **NOTE** > > The APIs of this module have been deprecated since API version 9. You are advised to use > [bundleManager-RemoteAbilityInfo](arkts-ability-remoteabilityinfo-depr-i-sys.md) instead. > > The APIs provided by this module are system APIs.

**Since:** 8

**Deprecated since:** 9

**Substitutes:** [remoteAbilityInfo:RemoteAbilityInfo](arkts-ability-remoteabilityinfo-depr-i-sys.md)

**System capability:** SystemCapability.BundleManager.DistributedBundleFramework

**System API:** This is a system API.

## elementName

```TypeScript
readonly elementName: ElementName
```

Element name information of the ability.

**Type:** ElementName

**Default:** Indicates the ability information

**Since:** 8

**Deprecated since:** 9

**Substitutes:** elementName

**System capability:** SystemCapability.BundleManager.DistributedBundleFramework

**System API:** This is a system API.

## icon

```TypeScript
readonly icon: string
```

Icon of the ability.

**Type:** string

**Default:** Indicates the icon of the ability

**Since:** 8

**Deprecated since:** 9

**Substitutes:** icon

**System capability:** SystemCapability.BundleManager.DistributedBundleFramework

**System API:** This is a system API.

## label

```TypeScript
readonly label: string
```

Ability name.

**Type:** string

**Default:** Indicates the label of the ability

**Since:** 8

**Deprecated since:** 9

**Substitutes:** label

**System capability:** SystemCapability.BundleManager.DistributedBundleFramework

**System API:** This is a system API.

