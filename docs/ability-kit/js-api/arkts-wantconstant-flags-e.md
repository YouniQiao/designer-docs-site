...
[Want.flags]./@ohos.app.ability.Want:Want.Flags字段常用的系统预置关键字。开发者可以通过这些预置关键字设置或获取应用跳转等场景中额外携带的标志位信息。
...
表示该URI可被接收方持久化。目标应用可以通过[fileShare.persistPermission]./@ohos.fileshare:fileShare.persistPermission接口进行权限持久化。
...
在多设备协同场景下，调用方应用通过DMS系统发起请求并且通过Flags字段携带此标志，协同方应用才会触发生命周期回调方法 [onCollaborate()]./@ohos.app.ability.UIAbility:UIAbility.onCollaborate。
...
表示是否关闭匹配失败弹窗功能。 通过[隐式方式拉起应用](docroot://application-models/app-startup-overview.md)时，如果没有能够匹配的应用，默认会弹出提示弹窗“暂无可用打开方式”。开发者可以通过该字段屏蔽该弹窗。
