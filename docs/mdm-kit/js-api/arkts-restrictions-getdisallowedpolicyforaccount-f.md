...
| feature | FeatureForAccount | Yes | 指定要查询的用户特性。 |
| accountId | number | Yes | 账号ID  取值应为≥0的整数。  - 用户ID，取值范围：大于等于0。 accountId可以通过  [getOsAccountLocalId]@ohos.account.osAccount:osAccount.AccountManager.getOsAccountLocalId(callback:  AsyncCallback)  等接口来获取。 |
