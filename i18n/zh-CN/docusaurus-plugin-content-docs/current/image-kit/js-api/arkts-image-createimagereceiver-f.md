...
**替代接口：** image.createImageReceiver(size:
...
| format | number | 是 | 图像格式，取值为[ImageFormat]image.ImageFormat常量（目前仅支持 ImageFormat:JPEG，实际返回格式由生产者决定，如相机  ）。 |
...
| format | ImageFormat | 是 | 图像格式，取值为[ImageFormat]image.ImageFormat常量（目前仅支持 ImageFormat:JPEG，实际返回格式由生产者决  定，如相机）。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | ImageReceiverOptions | 否 |  |
