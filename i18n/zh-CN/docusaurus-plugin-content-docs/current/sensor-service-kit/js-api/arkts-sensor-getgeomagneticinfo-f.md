...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| locationOptions | LocationOptions | 是 | Geographic location, including the longitude, latitude, and altitude. |
| timeMillis | long | 是 | Time when the magnetic declination is obtained. The value is a Unix timestamp, in ms. |
