...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| modifier | ContentModifier&lt;LoadingProgressConfiguration&gt; | 是 | 在LoadingProgress组件上，定制内容区的方法。<br/>modifier： 内容修改器，开发者需要自定义class实现ContentModifier接口。 |
...
设置LoadingProgress动画是否显示。LoadingProgress动画不显示时，该组件依旧占位。通用属性[Visibility](arkts-arkui-visibility-e.md#visibility).Hidden隐藏的是包括 [border](arkts-arkui-commonmethod-c.md#border-1)、[padding](arkts-arkui-commonmethod-c.md#padding-1)等整个组件范围，而enableLoading=false只隐藏 LoadingProgress本身动画内容，不包括border等。
