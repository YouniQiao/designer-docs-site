...
滑动选择器选项时，若选中项发生变化，触发该事件。 说明： - 如果某个选项有一半以上的区域进入选中项区域内，则该选项成为选中项。 - 选中项区域可通过设置[selectionIndicator](UIPickerComponentAttribute#selectionIndicator)进行标识。 如果设置选中项指示器为背景，则背景区域即为选中项区域。如果设置选中项指示器为分割线，则上下分割线的中心线内的区域 为选中项区域。
