...
用于播放视频文件并控制其播放状态的组件。 > **说明：** <br/> >  Video组件只提供简单的视频播放功能，无法支撑复杂的视频播控场景。复杂开发场景推荐使用[AVPlayer]{@link @ohos.multimedia.media:media.AVPlayer}播控API和[XComponent]{@link xcomponent}组件开发。<br/> >  Video组件在使用expandSafeArea扩展安全区域时，组件视频显示内容区域不支持扩展。
