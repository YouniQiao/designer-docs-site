...
**继承实现关系：** ScrollAttribute继承自：ScrollableCommonMethod<ScrollAttribute>。
...
| options | EdgeEffectOptions | 否 | 组件内容大小小于组件自身时，是否开启滑动效果。  设置为{ alwaysEnabled: true }会开启滑动效果，{ alwaysEnabled: false }不开启。  默认值：{ alwaysEnabled: true } [since 11] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | boolean | 是 | 是否支持滚动手势。设置为true时可以通过手指或者鼠标滚动，设置为false时无法通过手指或者鼠标滚动，但不影响控制器的滚动接口。 默认值：true |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | OffsetOptions | 是 | 初始滚动偏移量。当输入的大小为百分比时，  初始滚动偏移量为Scroll组件主轴方向大小与百分比数值之积。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scale | number | 是 | Scroll组件内容的最小手势缩放比例。  默认值：1。  取值范围：(0, maxZoomScale]。大于maxZoomScale时按maxZoomScale处理。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | ScrollOnScrollCallback | 是 | Scroll滚动时触发的回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | ScrollOnDidZoomCallback | 是 | 每帧缩放完成时回调。 |
...
**替代接口：** scroll/Scroll#onWillScroll
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | OnScrollEdgeCallback | 是 | 滚动到的边缘位置。 [since 18] |
...
**替代接口：** scroll/Scroll#onScrollStop
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | OnScrollFrameBeginCallback | 是 | 每帧滚动开始回调函数。 [since 18] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | VoidCallback | 是 | 滚动开始回调。 [since 18] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| event | VoidCallback | 是 | 滚动停止回调。 [since 18] |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| handler | ScrollOnWillScrollCallback | 是 | Scroll滚动前触发的回调。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | ScrollDirection | 是 | 滚动方向。 默认值：ScrollDirection.Vertical |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| barState | BarState | 是 | 滚动条状态。 默认值：BarState.Auto |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number \| string | 是 | 滚动条的宽度。 默认值：4 单位：vp  设置为小于0的值时，按默认值处理。设置为0时，不显示滚动条。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| value | number \| string \| Resource | 是 | 滚动条的宽度。  单位：vp  默认值：4  取值范围：[0, +∞)。设置为小于0的值时，按4vp处理。设置为0时，不显示滚动条。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| scale | number | 是 | 设置Scroll组件内容的缩放比例。  默认值：1。  取值范围：(0, +∞)。 |
