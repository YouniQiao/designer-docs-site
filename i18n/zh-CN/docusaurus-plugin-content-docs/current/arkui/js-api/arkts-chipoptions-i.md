...
设置组件激活状态下的系统材质样式。不同材质具有不同的效果，能够影响组件的[backgroundColor]CommonMethod#backgroundColor(value: ResourceColor)、 [border]CommonMethod#border、[shadow]CommonMethod#shadow(value: ShadowOptions | ShadowStyle)等视觉属性。 默认值：undefined 值为undefined时，不应用材质样式。
...
设置组件系统材质样式。不同材质具有不同的效果，能够影响组件的[backgroundColor]CommonMethod#backgroundColor(value: ResourceColor)、 [border]CommonMethod#border、[shadow]CommonMethod#shadow(value: ShadowOptions | ShadowStyle)等视觉属性。 默认值：undefined 值为undefined时，不应用材质样式。
...
Chip尺寸。 默认值：ChipSize.NORMAL SizeOptions类型参数不支持百分比设置，异常值按默认值处理。 **说明**：[适老化](docroot://ui/arkui-support-for-aging-adaptation.md)在size指定具体宽高时不生效，size设置为{ height: 0, width: 0 }除外。
