...
| [UIObserver](arkts-arkui-uiobserver-c.md) | 提供UI组件行为变化的无感监听能力。@link UIContext#getUIObserver}方法获取到UIObserver对象，再通过该对象调用对应方法。&gt;&gt; - UIObserver仅能监听到本进程内的相关信息，不支持获取&lt;!--Del--&gt;[UIExtensionComponent](../arkts-components/arkts-arkui-uiextensioncomponent.md)等&lt;!--DelEnd--&gt;跨进程场景的信&gt; 息。 |
