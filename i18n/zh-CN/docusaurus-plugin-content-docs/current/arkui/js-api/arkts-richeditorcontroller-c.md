...
**继承实现关系：** RichEditorController继承自：RichEditorBaseController。
...
| options | RichEditorBuilderSpanOptions | 否 |  |
...
| options | RichEditorImageSpanOptions | 否 |  |
...
| options | RichEditorSymbolSpanOptions | 否 |  |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| content | ResourceStr | 是 | 文本内容。 从API version 20开始，支持Resource类型。 [since 20] |
| options | RichEditorTextSpanOptions | 否 |  |
