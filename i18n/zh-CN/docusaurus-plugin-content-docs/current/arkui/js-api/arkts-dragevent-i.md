...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| keys | Array&lt;string> | 是 | 获取功能键按压状态。报错信息请参考以下错误码。支持功能键 'Ctrl' \| 'Alt' \| 'Shift'。 说明： 此接口不支持在手写笔场景  下使用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | DataSyncOptions | 是 | 获取拖拽数据时的参数，包含目标路径、文件冲突选项、进度条类型等。数据传输过程中可使用  [cancelDataLoading](arkts-dragcontroller-c.md#cancelDataLoading)接口取消数据加载。 |
