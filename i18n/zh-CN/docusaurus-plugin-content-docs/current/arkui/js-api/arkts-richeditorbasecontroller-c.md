...
**继承实现关系：** RichEditorBaseController实现：TextEditControllerEx。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| styledString | StyledString | 是 | 设置属性字符串样式的提示文本，其优先级高于[placeholder](arkts-richeditorattribute-c.md#placeholder)属性设  置的提示文本。 提示文本不支持触发属性字符串[GestureStyle](arkts-gesturestyle-c.md#GestureStyle)样式绑定的手势事件，以及  [UrlStyle](arkts-urlstyle-c.md#UrlStyle)样式的超链接跳转能力。 |
