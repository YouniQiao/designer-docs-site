...
| delay | number | 是 | Number of milliseconds delayed before the execution. It is recommended that an integer be  passed in; a decimal will be rounded down. If this parameter is omitted, the default value of 0 is used.NOTE 1. This timer is not a precise timer, and there may be a discrepancy between the actual delay and  the expected delay. 2. If the value is less than 1, it will be defaulted to 0. 3. The value is subject  to system limitations. If it exceeds 2^31 – 1, the value will be 0. |
