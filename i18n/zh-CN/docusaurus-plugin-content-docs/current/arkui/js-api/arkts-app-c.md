...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| options | RequestFullWindowOptions | 否 | Transition time from non-full window to full window, in milliseconds.  By default, the value is in direct proportion to the distance between the non-full window and the full window. |
