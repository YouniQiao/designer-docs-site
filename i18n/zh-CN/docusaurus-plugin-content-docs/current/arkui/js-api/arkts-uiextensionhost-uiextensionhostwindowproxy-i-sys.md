...
| callback | Callback&lt;{ type: window.AvoidAreaType, area: window.AvoidArea }> | 否 | 回调函数：如果传入该参数，则关闭该监听。如果未传入参  数，则关闭所有系统避让区变化的监听。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback&lt;uiExtension.AvoidAreaInfo> | 否 |  |
...
| callback | Callback&lt;{ type: window.AvoidAreaType, area: window.AvoidArea }> | 是 | 回调函数：入参用于接收当前避让区的信息，其中，"  type"表示窗口避让区类型，"area"表示窗口内容避让区域。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| callback | Callback&lt;uiExtension.AvoidAreaInfo> | 是 | - Callback used to return the area. |
