...
**替代接口：** huks.exportKeyItem(keyAlias:
...
| callback | AsyncCallback&lt;HuksResult> | 是 | 回调函数。当导出密钥成功时，err为undefined，data为获取到的HuksResult；否则为错误对象。HuksResult的  outData返回从密钥中导出的公钥。 |
