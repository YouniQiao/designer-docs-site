...
abort终止密钥操作。使用callback异步回调。 > **说明：** > > 从API version 8开始支持，从API version 9开始废弃，建议使用 > [huks.abortSession<sup>9+</sup>](arkts-universalkeystore-abortsession-f.md#abortsession-1) > 替代。
...
abort终止密钥操作。使用Promise异步回调。 > **说明：** > > 从API version 8开始支持，从API version 9开始废弃，建议使用 > [huks.abortSession<sup>9+</sup>](arkts-universalkeystore-abortsession-f.md#abortsession-2)替代。
