...
**替代接口：** deviceManager.bindDriverWithDeviceId(deviceId:
...
| callback | AsyncCallback&lt;RemoteDeviceDriver> | 是 | 回调函数。当绑定设备驱动成功时，err为undefined，data为包括设备ID和远程对象的  [RemoteDeviceDriver]deviceManager.RemoteDeviceDriver对象；否则为错误对象。 |
