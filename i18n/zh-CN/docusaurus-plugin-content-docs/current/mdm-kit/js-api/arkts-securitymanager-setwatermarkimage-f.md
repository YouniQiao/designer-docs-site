...
| accountId | number | 是 | 用户ID。accountId可以通过@ohos.account.osAccount中的  [getOsAccountLocalId](../../apis-basic-service-kit/arkts-apis/arkts-osaccount-accountmanager-i.md#getOsAccountLocalId)等接口来获取。@ohos.account.osAccount to obtain the account ID. |
...
| accountId | number | 是 | 系统账号ID  取值应为≥0的整数。 |
| properties | WatermarkProperties | 是 | 水印图片参数信息 |
