...
| startTime | number | 是 | 查询起始时间  单位为： 毫秒，取值应为≥0的整数。 |
| endTime | number | 是 | 查询结束时间，单位：毫秒（时间戳）  单位为： 毫秒，取值应为≥0的整数。 |
| accountId | number | 是 | accountId为操作系统帐户的本地ID。  取值应为≥0的整数。  - 用户ID，取值范围：大于等于0的整数。 accountId可以通过@ohos.account.osAccount中的  [getOsAccountLocalId](../../apis-basic-service-kit/arkts-apis/arkts-osaccount-accountmanager-i.md#getOsAccountLocalId)等接口来获取。 |
