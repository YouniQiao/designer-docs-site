...
| appIdentifiers | Array&lt;string> | 是 | 应用唯一标识符的数组。可以通过接口bundleManager.getInstalledBundleList获取bundleInfo.signatureInfo.appIdentifier。取值范围：数组长度不能超过200。  最大长度为200。 |
| accountId | number | 是 | 用户ID，取值范围：大于等于0。  - 用户ID，取值范围：大于等于0。  accountId可以通过@ohos.account.osAccount中的getOsAccountLocalId等接口来获取。 |
