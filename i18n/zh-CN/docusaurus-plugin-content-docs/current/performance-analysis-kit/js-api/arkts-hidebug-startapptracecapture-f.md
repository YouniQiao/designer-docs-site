...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| tags | long[] | 是 | Scope for trace collection. For details, see [tags]hidebug.tags. |
| flag | TraceFlag | 是 | For details, see [TraceFlag]hidebug.TraceFlag. |
| limitSize | int | 是 | Limit on the trace file size, in bytes. The maximum size of a single file is 500 MB. |
