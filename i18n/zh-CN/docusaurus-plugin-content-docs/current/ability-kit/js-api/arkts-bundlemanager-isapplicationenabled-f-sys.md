...
| appIndex | int | 是 | 表示分身应用的索引。 appIndex为0时，表示获取主应用的禁用或使能状态。appIndex大于0时，表示获取指定分身应用的禁用或使能状态。 |
...
| --- | --- | --- | --- |
| bundleName | string | 是 | 表示应用程序的bundleName。 |
| callback | AsyncCallback&lt;boolean> | 是 | [AsyncCallback](../../apis-basic-service-kit/arkts-apis/arkts-asynccallback-i.md#AsyncCallback)，返回true表示当前应用为使能状态，返回  false表示应用为禁用状态。 |
