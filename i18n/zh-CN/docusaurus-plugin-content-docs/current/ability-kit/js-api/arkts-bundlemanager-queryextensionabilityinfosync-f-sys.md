...
| extensionAbilityFlags | int | 是 | 表示返回的ExtensionInfo对象中需要包含的信息标志，具体取值及不同含义参考  [ExtensionAbilityFlag]bundleManager.ExtensionAbilityFlag。 |
| userId | int | 否 | 表示用户ID，可以通过  [getOsAccountLocalId]@ohos.account.osAccount:osAccount.AccountManager.getOsAccountLocalId(callback: AsyncCallback)  获取，默认值：调用方所在用户，取值范围：大于等于0。 |
...
| extensionAbilityType | string | 是 | 表示自定义extensionAbility的类型。 |
| extensionAbilityFlags | int | 是 | 表示返回的ExtensionInfo对象中需要包含的信息标志，具体取值及不同含义参考  [ExtensionAbilityFlag]bundleManager.ExtensionAbilityFlag。 |
| userId | int | 否 | 表示用户ID，可以通过  [getOsAccountLocalId]@ohos.account.osAccount:osAccount.AccountManager.getOsAccountLocalId(callback: AsyncCallback)  获取，默认值：调用方所在用户ID。取值范围：大于等于0。 |
