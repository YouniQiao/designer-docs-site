...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | string | 是 | 要设置的应用类型，支持取值包括：  [ApplicationType](arkts-defaultappmanager-applicationtype-e.md#ApplicationType)中的值、  [MIMEType](docroot://database/uniform-data-type-list.md#基础类型)类型、或  [UniformDataType](../../apis-arkdata/arkts-apis/arkts-uniformtypedescriptor-uniformdatatype-e.md#UniformDataType)类型。 |
| elementName | ElementName | 是 | 要设置为默认应用的组件信息，仅使用其中的bundleName、abilityName、moduleName属性，且三个属性必须设置。 |
| appIndex | int | 是 | 表示分身应用的索引。 取值范围：1、2、3、4、5。 |
| userId | int | 否 |  |
