...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| startupTask | string | 是 | 启动任务[StartupTask](arkts-startuptask-c.md#StartupTask)的名称或预加载so名称。 |

**返回值：**

| 类型 | 说明 |
| --- | --- |
| Object | 输入为启动任务名时，返回指定的启动任务  [init](arkts-startuptask-c.md#init)返回的执行结果。 输入为so  文件名时，返回undefined。 |
