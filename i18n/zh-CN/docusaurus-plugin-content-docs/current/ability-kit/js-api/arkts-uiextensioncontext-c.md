...
**继承实现关系：** UIExtensionContext继承自：ExtensionContext。
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| connection | long | 是 | 连接的ServiceExtensionAbility的标识Id，即  [connectServiceExtensionAbility](arkts-uiextensioncontext-c.md#connectServiceExtensionAbility)返回的connectionId。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| proxy | UIServiceProxy | 是 | [connectUIServiceExtensionAbility](arkts-uiextensioncontext-c.md#connectUIServiceExtensionAbility)返回的Proxy。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| colorMode | ConfigurationConstant.ColorMode | 是 | 设置颜色模式，包括： - COLOR_MODE_DARK：深色模式 - COLOR_MODE_LIGHT：浅色模  式 - COLOR_MODE_NOT_SET：不设置（跟随系统或应用） |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| isForbidden | boolean | 是 | 是否允许[UIExtensionAbility](arkts-uiextensionability-c.md#UIExtensionAbility)  拉起的页面被使用方的页面覆盖。true表示不允许，false表示允许。 |
...
| --- | --- | --- | --- |
| want | Want | 是 | 启动ServiceExtensionAbility的Want信息。 |
| accountId | int | 是 | 系统账号的ID，可以通过  [getOsAccountLocalId]@ohos.account.osAccount:osAccount.AccountManager.getOsAccountLocalId(callback: AsyncCallback)  接口获取。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| wantList | Array&lt;Want> | 是 | 需要被同时拉起的多个UIAbility的启动参数列表，最多支持传入4个Want。启动参数Want不支持隐式启动、跨用户启动、分布式、免安装和按需加载，不指明分身的情况下默认  启动主应用。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| primaryWindowId | int | 是 | 启动第一个UIAbility的主窗的窗口ID。窗口ID是  [WindowProperties](docroot://reference/apis-arkui/arkts-apis-window-i.md#windowproperties)的属性，WindowProperties可通过  [getWindowProperties()](docroot://reference/apis-arkui/arkts-apis-window-Window.md#getwindowproperties9)获取。 |
