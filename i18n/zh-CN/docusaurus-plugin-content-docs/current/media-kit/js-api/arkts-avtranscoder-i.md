...
| callback | Callback&lt;int> | 否 | 已注册的进度更新事件回调。由于当前回调注册时，仅会保留最后一次注册的回调，建议此参数缺省。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| type | 'error' | 是 | 转码错误事件回调类型'error'。 - 'error'：录制过程中发生错误，触发该事件。 |
...
| callback | Callback&lt;int> | 是 | 回调函数，返回进度更新事件，函数中的参数number，表示当前转码进度。 |
