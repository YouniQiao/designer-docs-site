...
查询当前[ContinuousTaskRequest](arkts-backgroundtasks-continuoustaskrequest-c.md#continuoustaskrequest)设置的长时任务主类型，是否支持申请长时任务。是否支持申请长时任务请参考 [BackgroundTaskMode](arkts-backgroundtasks-backgroundtaskmode-e.md#backgroundtaskmode)的说明。
...
请求用户授权是否能在后台长时间运行，使用callback异步回调。接口调用成功会弹出用户授权弹框，建议应用在前台时调用该接口，提示用户进行授权。仅适用于特殊场景类型 [MODE_SPECIAL_SCENARIO_PROCESSING](arkts-backgroundtasks-backgroundtaskmode-e.md#backgroundtaskmode)的长时任务。
...
| --- | --- | --- | --- |
| context | Context | 是 | 应用运行的上下文。<br><br>FA模型的应用Context定义见[Context]{@link./app/context}。<br>Stage模型的应用Context定义见[Context](../../apis-ability-kit/arkts-apis/arkts-ability-context-c.md#context)。 <br> **说明：** Stage模型中，仅支持UIAbility申请；FA模型中，仅支持ServiceAbility申请。 |
| callback | Callback&lt;UserAuthResult&gt; | 是 | 用户操作后，返回授权结果。 |
...
| callback | Callback&lt;UserAuthResult&gt; | 是 | The callback of the function. |
...
是否合并通知，true表示合并，false表示不合并，默认为false。 **说明：** 该属性在 [updateBackgroundRunning](arkts-backgroundtasks-updatebackgroundrunning-f.md#updatebackgroundrunning-2) 接口中不生效，如需在已有任务上合并通知，请重新申请该任务，并在申请时设置为支持合并。
...
长时任务ID，默认值为-1。 **说明：** 如果combinedTaskNotification取值为true，则该值为必填项，且必须是存在的ID。 作为 [updateBackgroundRunning](arkts-backgroundtasks-updatebackgroundrunning-f.md#updatebackgroundrunning-2) 接口入参时，该属性必填，且必须是存在的ID。 可以通过 [getAllContinuousTasks](arkts-backgroundtasks-getallcontinuoustasks-f.md#getallcontinuoustasks-2) 接口查看当前所有长时任务信息。
