...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| timeout | int | 是 | 锁定和持有RunningLock的时长，单位：毫秒。 该参数必须为数字类型： -1：永久持锁，需要主动释放。 0：默认3s后超时释放。  >0：按传入值超时释放。 |
...
**替代接口：** runningLock.RunningLock.isHolding
...
**替代接口：** runningLock.RunningLock.hold
...
**替代接口：** runningLock.RunningLock.unhold
