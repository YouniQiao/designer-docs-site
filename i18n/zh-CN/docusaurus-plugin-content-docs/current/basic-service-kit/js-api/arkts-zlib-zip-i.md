...
| dest | ArrayBuffer | 是 | 目标缓冲区。 |
| source | ArrayBuffer | 是 | 源数据缓冲区。 |
| level | CompressLevel | 是 | 参考[CompressLevel枚举定义]zlib.CompressLevel。 |
| sourceLen | long | 否 | 源数据长度。默认值为0。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| strm | ZStream | 是 | 参考[ZStream定义]zlib.ZStream。 |
| flush | CompressFlushMode | 是 | 参考[CompressFlushMode定义]zlib.CompressFlushMode。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| source | Zip | 是 | 参考[Zip定义]zlib.Zip。 |
...
| strm | ZStream | 是 | 参考[ZStream定义]zlib.ZStream。 |
| level | CompressLevel | 是 | 参考[CompressLevel枚举定义]zlib.CompressLevel。 |
| method | CompressMethod | 是 | 参考[CompressMethod枚举定义]zlib.CompressMethod。 |
| windowBits | int | 是 | 控制内存窗口的大小，并指定数据的格式（zlib、gzip、raw deflate）。取值如下： zlib格式：[1, 15]。 gzip格式：大于15  。 raw deflate格式：[-15, -1]。 |
| memLevel | MemLevel | 是 | 参考[MemLevel枚举定义]zlib.MemLevel。 |
| strategy | CompressStrategy | 是 | 参考[CompressStrategy枚举定义]zlib.CompressStrategy。 |
...
| strm | ZStream | 是 | 参考[ZStream定义]zlib.ZStream。 |
| backIn | InflateBackInputCallback | 是 | 一种函数，用于从末尾解压缩数据，以从输入源读取原始压缩数据。 |
| inDesc | RecordData | 是 | 通用对象。 |
| backOut | InflateBackOutputCallback | 是 | 将解压缩的数据写入目标输出。 |
| outDesc | RecordData | 是 | 通用对象。 |
...
| --- | --- | --- | --- |
| strm | ZStream | 是 | 参考[ZStream定义]zlib.ZStream。 |
| windowBits | long | 是 | 控制内存窗口的大小，并指定数据的格式（zlib、gzip、raw deflate）。取值如下： zlib格式：[1, 15]。 gzip格式：大于1  5。 raw deflate格式：[-15, -1]。 |
