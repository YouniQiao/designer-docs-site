...
| mode | string | 是 | 用于指定访问模式。详情与[gzopen]zlib.GZip.gzopen一致。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| flush | CompressFlushMode | 是 | 控制刷新操作的行为，参考[CompressFlushMode枚举]zlib.CompressFlushMode的定义。 |
...
| whence | OffsetReferencePoint | 是 | 定义偏移的参考点，参考[OffsetReferencePoint枚举定义]zlib.OffsetReferencePoint。 |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| level | CompressLevel | 是 | 压缩级别，参考[CompressLevel枚举定义]zlib.CompressLevel。 |
| strategy | CompressStrategy | 是 | 压缩策略，参考[CompressStrategy枚举定义]zlib.CompressStrategy。 |
