...
上传任务返回码。返回0表示上传任务成功，返回其它值表示上传任务失败，具体请参见message参数中的上传任务结果描述信息。 此处推荐使用 [request.agent.create]request.agent.create(context: BaseContext, config: Config)创建上传 任务，并获取标准错误码处理异常分支。
