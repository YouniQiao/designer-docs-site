...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| bytes | Uint8Array | 是 | Input byte stream. To detect the encoding of a text string,  convert the text to a byte stream first while preserving its original format.  Byte stream to be identified and encoded |
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| ch | string | 是 | Input character. If the input is a string, only the type of the first character is  checked. [since 12] |
