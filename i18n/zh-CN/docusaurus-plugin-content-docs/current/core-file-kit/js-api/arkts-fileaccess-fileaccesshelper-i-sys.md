...
**替代接口：** @ohos.file.fs:AtomicFile#delete
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| uri | string | 是 | 所选文件或目录的uri（从[FileInfo]fileAccess.FileInfo中获取）。 |
| metaJson | string | 是 | json字符串，包含查询属性[FILEKEY]fileAccess.FileKey 。 |
...
| uri | string | 是 | Indicates the selected file or directory. |
| displayName | string | 是 | Indicates the new directory or file name. |
| callback | AsyncCallback&lt;string> | 是 | The callback is used to return a URI representing the new file or  directory. |
