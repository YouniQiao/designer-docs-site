...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| plaintextFd | number | 是 | 待加密明文文件的fd。取值范围为[0, 231-1]。当fd小于0时，打印错误日志，函数停止运行；当fd大于231-1  时，fd的值被截断。 |
| ciphertextFd | number | 是 | 目标加密文件的fd。取值范围为[0, 231-1]。当fd小于0时，打印错误日志，函数停止运行；当fd大于231-1  时，fd的值被截断。 |
