...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| fd | number | 是 | 待查询文件的fd（文件描述符）。取值范围为[0, 231-1]。当fd小于0时，抛出错误码19100001；当fd大于231-1时，fd  的值被截断。 |
...
| --- | --- | --- | --- |
| fd | number | 是 | 待查询文件的fd（文件描述符）。取值范围为[0, 231-1]。当fd小于0时，抛出错误码19100001；当fd大于231-1时，fd  的值被截断。 |
| callback | AsyncCallback&lt;boolean> | 是 | 回调函数，用于接收查询结果。回调参数包括：err（错误对象，查询成功时为undefined）和res（查询结果，返回true表示是DLP  文件，返回false表示非DLP文件）。 |
