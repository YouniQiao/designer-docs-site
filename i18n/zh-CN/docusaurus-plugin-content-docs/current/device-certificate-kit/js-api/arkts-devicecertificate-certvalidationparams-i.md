...
验证日期。格式为YYMMDDHHMMSSZ或YYYYMMDDHHMMSSZ，默认使用当前系统时间。 支持自定义验证时间，适用于离线验证历史签名等场景。
...
主机名列表。验证证书的主题备用名（SAN）或通用名（CN）是否包含指定的主机名。最大个数：100，每个主机名最大长度：128。 只要匹配其中一个主机名即校验成功。
