...
CmsGenerator对象用于生成CMS（Cryptographic Message Syntax）格式的消息。 > **说明：** > > PKCS#7是用于存储签名或加密数据的标准语法。注意CMS是PKCS#7的扩展，PKCS#7支持的数据类型包括数据、签名数据、信封数据、 > 签名和信封数据、摘要数据、加密数据。常用于保护数据的完整性和机密性。
...
| 类型 | 说明 |
| --- | --- |
| Promise&lt;Uint8Array \| string&gt; | Promise对象，返回CMS数据。 |
